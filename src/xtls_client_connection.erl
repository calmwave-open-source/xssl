...
initial_hello({call, From}, {start, Timeout},
    ...
    KeyShare = xtls_client_connection_1_3:maybe_generate_client_shares(SslOpts),
    ...
    {UseTicket, State1} = xtls_client_connection_1_3:maybe_automatic_session_resumption(State0),
    TicketData = xtls_handshake_1_3:get_ticket_data(self(), SessionTickets, UseTicket),
    OcspNonce = xtls_handshake:ocsp_nonce(SslOpts),
    Hello0 = xtls_handshake:client_hello(Host, Port, ConnectionStates0, SslOpts,
    ...
    Hello1 = xtls_handshake_1_3:maybe_add_early_data_indication(Hello0,
                                                               EarlyData,
                                                               HelloVersion),

    %% Update pre_shared_key extension with binders (TLS 1.3)
    Hello2 = xtls_handshake_1_3:maybe_add_binders(Hello1, TicketData, HelloVersion),
    ...
    RequestedVersion = xtls_record:hello_version(Versions),
    ...
    try
        ...
        {Ref, #alert{} = Alert} ->
            ...
            xssl_gen_statem:handle_own_alert(Alert, init, NewState)
    end;
...
initial_hello(Event, Msg, State) ->
    xssl_gen_statem:initial_hello(Event, Msg, State).
        ...
        case xtls_handshake:hello(Hello, SslOptions, ConnectionStates0, Renegotiation, OldId) of
            ...
            {next_state, wait_sh, SelectedVersion, StaplingState} ->
                ...
                {next_state, wait_sh,
                 ...
                 [{change_callback_module, xtls_client_connection_1_3},
                  {next_event, internal, Hello}]}
        end
    catch throw:#alert{} = Alert ->
            xssl_gen_statem:handle_own_alert(Alert, ?STATE(hello), State)
    end;
hello(info, Event, State) ->
    xtls_gen_connection:gen_info(Event, ?STATE(hello), State);
...
abbreviated(info, Event, State) ->
    xtls_gen_connection:gen_info(Event, ?STATE(abbreviated), State);
...
wait_stapling(info, Event, State) ->
    xtls_gen_connection:gen_info(Event, ?STATE(wait_stapling), State);
...
certify(info, Event, State) ->
    xtls_gen_connection:gen_info(Event, ?STATE(certify), State);
...
cipher(info, Event, State) ->
    xtls_gen_connection:gen_info(Event, ?STATE(cipher), State);
...
connection(info, Event, State) ->
    xtls_gen_connection:gen_info(Event, ?STATE(connection), State);
...
connection(internal, #hello_request{},
    ...
    try xtls_sender:peer_renegotiate(Pid) of
        {ok, Write} ->
            ...
            {State, Actions} =
                xtls_gen_connection:send_handshake(Hello,
                                                  State0#state{connection_states =
                                                                   ConnectionStates#{current_write
                                                                                     => Write},
                                                               session = Session}),
            xtls_gen_connection:next_event(hello, no_record, State, Actions)
        catch
            _:Reason:ST ->
...
connection(internal, #hello_request{},
    ...
    {State, Actions} = xtls_gen_connection:send_handshake(Hello, State0),
    xtls_gen_connection:next_event(hello, no_record, State, Actions);
...
downgrade(Type, Event, State) ->
     xssl_gen_statem:downgrade(Type, Event, State).
...
next_statem_state([Version]) ->
    case xssl:tls_version(Version) of
        ?TLS_1_3 ->
            wait_sh;
        _  ->
            hello
