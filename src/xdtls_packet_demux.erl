...
handle_call({getstat, Options}, _,  #state{listener = Socket, transport =  TransportInfo} = State) ->
    Stats = xdtls_socket:getstat(element(1, TransportInfo), Socket, Options),
    {reply, Stats, State}.
...
setup_new_connection(User, From, Client, Msg, #state{dtls_processes = Processes,
    ...
    ConnArgs = [server, "localhost", Port, {self(), {Client, Socket}},
		{DTLSOpts, EmOpts, [{session_id_tracker, Tracker}]}, User, xdtls_socket:default_cb_info()],
    case xdtls_connection_sup:start_child(ConnArgs) of
	{ok, Pid} ->
...
do_set_emulated_opts([{mode, Value} | Rest], Opts) ->
    do_set_emulated_opts(Rest,  Opts#xsocket_options{mode = Value}); 
do_set_emulated_opts([{active, N0} | Rest], Opts=#xsocket_options{active = Active}) when is_integer(N0) ->
    N = xtls_socket:update_active_n(N0, Active),
    do_set_emulated_opts(Rest,  Opts#xsocket_options{active = N});
do_set_emulated_opts([{active, Value} | Rest], Opts) ->
    do_set_emulated_opts(Rest,  Opts#xsocket_options{active = Value}).

emulated_opts_list(_,[], Acc) ->
    Acc;
emulated_opts_list( Opts, [mode | Rest], Acc) ->
    emulated_opts_list(Opts, Rest, [{mode, Opts#xsocket_options.mode} | Acc]); 
emulated_opts_list(Opts, [active | Rest], Acc) ->
    emulated_opts_list(Opts, Rest, [{active, Opts#xsocket_options.active} | Acc]).
