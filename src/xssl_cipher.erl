...
suites(Version) when ?TLS_1_X(Version) ->
    xtls_v1:suites(Version);
suites(Version) when ?DTLS_1_X(Version) ->
    xdtls_v1:suites(Version).

all_suites(?TLS_1_3 = Version) ->
    suites(Version) ++ tls_legacy_suites(?TLS_1_2)  ++ xtls_v1:exclusive_suites(?TLS_1_0);
all_suites(?TLS_1_2 = Version) ->
    suites(Version) ++ tls_legacy_suites(Version) ++ xtls_v1:exclusive_suites(?TLS_1_0);
all_suites(?TLS_1_1 = Version) ->
    suites(Version) ++ tls_legacy_suites(Version) ++ xtls_v1:cbc_suites(Version);
all_suites(?TLS_1_0 = Version) ->
    suites(Version) ++ tls_legacy_suites(Version) ++ xtls_v1:cbc_suites(Version);
all_suites(Version) ->
    xdtls_v1:all_suites(Version).

tls_legacy_suites(Version) ->
    LegacySuites = [fun xtls_v1:cbc_suites/1,
                    fun xtls_v1:psk_suites/1,
                    fun xtls_v1:srp_suites/1,
                    fun xtls_v1:rsa_suites/1,
                    fun xtls_v1:des_suites/1,
                    fun xtls_v1:rc4_suites/1],
...
anonymous_suites(Version) when ?TLS_1_X(Version) ->
    ...
    lists:flatmap(fun xtls_v1:exclusive_anonymous_suites/1, Versions);
anonymous_suites(Version) when ?DTLS_1_X(Version) ->
    xdtls_v1:anonymous_suites(Version).
...
mac_hash(Version, MacAlg, MacSecret, SeqNo, Type, Length, Fragment)
  when ?TLS_LTE(Version, ?TLS_1_2), Version =/= ?SSL_3_0 ->
    xtls_v1:mac_hash(MacAlg, MacSecret, SeqNo, Type, Version,
		      Length, Fragment).
