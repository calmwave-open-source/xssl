...
listen(Port, #xconfig{inet_ssl = SockOpts,
                     ssl = SslOpts,
                     emulated = EmOpts,
                     inet_user = Options} = Config) ->
    IP = proplists:get_value(ip, SockOpts, default_ip(SockOpts)),
    case xdtls_listener_sup:lookup_listener(IP, Port) of
        undefined ->
            start_new_listener(IP, Port, self(), Config);
        {ok, Listener} ->
            xdtls_packet_demux:new_owner(Listener, self()),
            xdtls_packet_demux:set_all_opts(
              Listener, {Options,
                          emulated_socket_options(EmOpts,
                                                  #xsocket_options{}),
                          SslOpts}),
            xdtls_listener_sup:register_listener({self(), Listener},
                                                IP, Port),
            {ok, create_xdtls_socket(Config, Listener, Port)};
        Error ->
            Error
    end.

accept({Listener,_}, #xconfig{}, _Timeout) ->
    case xdtls_packet_demux:accept(Listener, self()) of
	{ok, Pid} ->
            receive
...
connect(Address, Port, #xconfig{transport_info = {Transport, _, _, _, _} = CbInfo,
	...
	{ok, Socket} ->
	    xssl_gen_statem:connect(ConnectionCb, Address, Port, {{Address, Port},Socket},
				   {SslOpts, 
				    emulated_socket_options(EmOpts, #xsocket_options{}), undefined},
				   self(), CbInfo, Timeout);
	{error, _} = Error->	
	    Error
    end.

close_listen(#xsslsocket{socket_handle = {Pid, Port0},
                        listener_config = #xconfig{inet_ssl = SockOpts}}, Timeout) ->
    IP = proplists:get_value(ip, SockOpts, default_ip(SockOpts)),
    Port = get_real_port(Pid, Port0),
    xdtls_listener_sup:register_listener({undefined, Pid}, IP, Port),
    case xdtls_packet_demux:close(Pid) of
        stop ->
...
socket([Pid], gen_udp = Transport,
    ...
    #xsslsocket{socket_handle = PeerAndSock,
...
socket([Pid], Transport, Socket, ConnectionCb, Tab) when Tab =/= undefined ->
    #xsslsocket{socket_handle = Socket,
               connection_handler = Pid,
               payload_sender = Pid,
               transport_cb = Transport,
               connection_cb = ConnectionCb, 
...
setopts(_, Socket = #xsslsocket{socket_handle = {ListenPid, _},
                               listener_config = #xconfig{}}, Options) ->
    SplitOpts = {_, EmOpts} = xtls_socket:split_options(Options),
    check_active_n(EmOpts, Socket),
    xdtls_packet_demux:set_sock_opts(ListenPid, SplitOpts);
...
check_active_n(EmulatedOpts, Socket = #xsslsocket{socket_handle = {ListenPid, _},
                                                 listener_config = #xconfig{}}) ->
    ...
    case proplists:lookup(active, EmulatedOpts) of
        ...
        {_, N} when is_integer(N) ->
            {ok, #xsocket_options{active = Active}, _} = xdtls_packet_demux:get_all_opts(ListenPid),
            case Active of
...
getopts(_, #xsslsocket{socket_handle = {ListenPid, _}, listener_config =#xconfig{}}, Options) ->
    SplitOpts = xtls_socket:split_options(Options),
    xdtls_packet_demux:get_sock_opts(ListenPid, SplitOpts);
%%% Following clauses will not be called for emulated options, they are  handled in the connection process
...
getstat(gen_udp, Pid, Options) when is_pid(Pid) ->
    xdtls_packet_demux:getstat(Pid, Options);
...
emulated_socket_options(InetValues, #xsocket_options{
				       mode   = Mode,
                                       packet = Packet,
...
get_real_port(Listener, Port0) when is_pid(Listener) andalso
    ...
    case Port0 of
        0 ->
            {ok, {_, NewPort}} = xdtls_packet_demux:sockname(Listener),
            NewPort;
        _ ->
            Port0
    end.
...
start_new_listener(IP, Port0, Owner,
                   #xconfig{transport_info = {TransportModule, _,_,_,_},
                           inet_user = Options} = Config) ->
...
start_dtls_packet_demux(#xconfig{
                           transport_info =
                               {TransportModule, _,_,_,_} = TransportInfo,
                           emulated = EmOpts0,
                           ssl = SslOpts} = Config, IP, Port, Socket, Owner) ->
    EmOpts = emulated_socket_options(EmOpts0, #xsocket_options{}),
    case xdtls_listener_sup:start_child([Owner, Port, TransportInfo, EmOpts, SslOpts, Socket]) of
        {ok, Multiplexer} ->
            ok = TransportModule:controlling_process(Socket, Multiplexer),
            xdtls_listener_sup:register_listener({self(), Multiplexer},
                                                IP, Port),
            DTLSSocket = create_xdtls_socket(Config, Multiplexer, Port),
	    {ok, DTLSSocket};
        Error ->
            Error
    end.

create_xdtls_socket(#xconfig{emulated = EmOpts} = Config,
                   Listener, Port) ->
    Socket = #xsslsocket{
