...
start_link_dist() ->
    supervisor:start_link({local, xdtls_connection_sup_dist}, ?MODULE, []).
...
init(_) ->    
    ...
    ChildSpecs = [#{id       => undefined,
                    start    => {xssl_gen_statem, start_link, []},
                    restart  => temporary,
                    shutdown => 4000,
                    modules  => [xssl_gen_statem, xdtls_connection],
                    type     => worker
                   }
                 ], 
