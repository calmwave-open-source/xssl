...
-module(xtls_server_connection_1_3).
-moduledoc false.
...
init([?SERVER_ROLE, Sender, Tab, Host, Port, Socket, Options,  User, CbInfo]) ->
    State0 = #state{protocol_specific = Map} =
        xtls_gen_connection_1_3:initial_state(?SERVER_ROLE, Sender, Tab,
    ...
    try
	State = xssl_gen_statem:init_ssl_config(State0#state.ssl_options, ?SERVER_ROLE, State0),
...
initial_hello(Type, Event, State) ->
    xtls_server_connection:initial_hello(Type, Event, State).
...
user_hello({call, From}, cancel, State) ->
    ...
    xssl_gen_statem:handle_own_alert(?ALERT_REC(?FATAL, ?USER_CANCELED, user_canceled),
                                    user_hello, State);
user_hello({call, From}, {handshake_continue, NewOptions, Timeout},
           #state{handshake_env = #handshake_env{continue_status = {pause, ClientVersions}},
                  ssl_options = Options0} = State0) ->
    try xssl:update_options(NewOptions, ?SERVER_ROLE, Options0) of
        Options = #{versions := Versions} ->
            State1 = xssl_gen_statem:ssl_config(Options, ?SERVER_ROLE, State0),
            ...
            case ssl_handshake:select_supported_version(ClientVersions, Versions) of
                ...
                undefined ->
                    xssl_gen_statem:handle_own_alert(?ALERT_REC(?FATAL, ?PROTOCOL_VERSION),
                                                    ?STATE(user_hello), State);
                _Else ->
                    {next_state, hello, State,
                     [{change_callback_module, xtls_server_connection},
...
start(internal = Type, #change_cipher_spec{} = Msg,
    ...
    case ssl_handshake:init_handshake_history() of
        ...
        _ ->
            xtls_gen_connection_1_3:handle_change_cipher_spec(Type, Msg, ?STATE(start), State)
        end;
...
start(internal, #client_hello{extensions = #{client_hello_versions :=
    ...
    case xtls_record:is_acceptable_version(?TLS_1_3, ClientVersions) of
        true ->
...
negotiated(internal = Type, #change_cipher_spec{} = Msg, State) ->
    xtls_gen_connection_1_3:handle_change_cipher_spec(Type, Msg, ?STATE(negotiated), State);
negotiated(internal, {start_handshake, _} = Message, State0) ->
    case send_hello_flight(Message, State0) of
        #alert{} = Alert ->
            xssl_gen_statem:handle_own_alert(Alert, ?STATE(negotiated), State0);
        {State, NextState} ->
            {next_state, NextState, State, []}
    end;
negotiated(info, Msg, State) ->
    xtls_gen_connection:handle_info(Msg, ?STATE(negotiated), State);
negotiated(Type, Msg, State) ->
    xssl_gen_statem:handle_common_event(Type, Msg, ?STATE(negotiated), State).
...
wait_finished(internal = Type, #change_cipher_spec{} = Msg, State) ->
    xtls_gen_connection_1_3:handle_change_cipher_spec(Type, Msg, ?STATE(wait_finished), State);
wait_finished(internal,
    ...
    try
        ...
        {Record, #state{protocol_specific = PS} = State} = xssl_gen_statem:prepare_connection(State5, xtls_gen_connection),
        ...
        {Ref, #alert{} = Alert} ->
            xssl_gen_statem:handle_own_alert(Alert, ?STATE(wait_finished), State0)
    end;
...
wait_eoed(info, Msg, State) ->
    xtls_gen_connection:handle_info(Msg, ?STATE(wait_eoed), State);
wait_eoed(Type, Msg, State) ->
    xssl_gen_statem:handle_common_event(Type, Msg, ?STATE(wait_eoed), State).
...
do_handle_client_hello(#client_hello{cipher_suites = ClientCiphers,
    ...
    try
        ...
        ClientSignAlgsCert = xtls_handshake_1_3:get_signature_scheme_list(
                               maps:get(signature_algs_cert, Extensions, undefined)),
               CertAuths = xtls_handshake_1_3:get_certificate_authorities(maps:get(certificate_authorities,
        ...
        State3 = case maps:get(keep_secrets, Opts, false) of
                     true ->
                         xtls_handshake_1_3:set_client_random(State2, Hello#client_hello.random);
                     false ->
                         State2
                 end,

        State4 = xtls_handshake_1_3:update_start_state(State3,
                                                      #{cipher => Cipher,
                                                        key_share => KeyShare,
                                                        session_id => SessionId,
                                                        group => Group,
...
send_hello_flight({start_handshake, PSK0},
    ...
    try
        %% Create server_hello
        ServerHello = xtls_handshake_1_3:server_hello(server_hello, SessionId,
                                                     KeyShare, PSK0, ConnectionStates0),
        State1 = Connection:queue_handshake(ServerHello, State0),
        %% D.4.  Middlebox Compatibility Mode
        State2 = xtls_gen_connection_1_3:maybe_queue_change_cipher_spec(State1, last),

        PSK = xtls_handshake_1_3:get_pre_shared_key(PSK0, HKDF),

        State3 =
            xtls_handshake_1_3:calculate_handshake_secrets(ClientPublicKey,
        ...
        EncryptedExtensions = xtls_handshake_1_3:encrypted_extensions(State4),
        ...
        %% Create Finished
        Finished = xtls_handshake_1_3:finished(State8),
...
session_resumption({#state{ssl_options = #{session_tickets := Tickets},
  ...
  when Tickets =/= disabled -> % Resumption but early data prohibited
    State1 = xtls_gen_connection_1_3:handle_resumption(State0, ok),
...
new_session_ticket(Tracker, HKDF, RMS,
    ...
    xtls_server_session_ticket:new(Tracker, HKDF, RMS, PeerCert);
new_session_ticket(Tracker, HKDF, RMS,
                   #state{ssl_options = #{session_tickets := stateless_with_cert},
                          session = #session{peer_certificate = PeerCert}}) ->
    xtls_server_session_ticket:new(Tracker, HKDF, RMS, PeerCert);
new_session_ticket(Tracker, HKDF, RMS, _) ->
    xtls_server_session_ticket:new(Tracker, HKDF, RMS, undefined).
...
select_server_cert_key_pair(Session, [#{private_key := Key, certs := [Cert| _] = Certs} | Rest],
                            ...
                            Default0) ->
    {_, SignAlgo, SignHash, _, _} = xtls_handshake_1_3:get_certificate_params(Cert),
...
send_hello_retry_request(#state{connection_states = ConnectionStates0,
    ...
    ServerHello0 = xtls_handshake_1_3:server_hello(hello_retry_request, SessionId,
                                                  KeyShare, undefined, ConnectionStates0),
    {State1, ServerHello} = xtls_handshake_1_3:maybe_add_cookie_extension(State0, ServerHello0),

    State2 = Connection:queue_handshake(ServerHello, State1),
    %% D.4.  Middlebox Compatibility Mode
    State3 = xtls_gen_connection_1_3:maybe_queue_change_cipher_spec(State2, last),
    {State4, _} = Connection:send_handshake_flight(State3),

    %% Update handshake history
    State5 = xtls_handshake_1_3:replace_ch1_with_message_hash(State4),
...
maybe_send_certificate_request(#state{static_env = #static_env{protocol_cb = Connection,
    ...
    CertificateRequest = xtls_handshake_1_3:certificate_request(SignAlgs, SignAlgsCert,
                                                               CertDbHandle,
...
maybe_send_certificate(#state{session = #session{own_certificates = OwnCerts},
    ...
    case xtls_handshake_1_3:certificate(OwnCerts, CertDbHandle, CertDbRef, <<>>, server) of
        {ok, Certificate} ->
...
maybe_send_certificate_verify(#state{session = #session{sign_alg = SignatureScheme,
    ...
    case xtls_handshake_1_3:certificate_verify(CertPrivateKey, SignatureScheme, State, server) of
...
select_cipher_suite(false, [Cipher|ClientCiphers], ServerCiphers) ->
    case lists:member(Cipher, xtls_v1:exclusive_suites(?TLS_1_3)) andalso
