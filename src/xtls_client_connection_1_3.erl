...
-module(xtls_client_connection_1_3).
-moduledoc false.
...
init([?CLIENT_ROLE, Sender, Tab, Host, Port, Socket, Options,  User, CbInfo]) ->
    State0 = #state{protocol_specific = Map} =
        xtls_gen_connection_1_3:initial_state(?CLIENT_ROLE, Sender, Tab,
                                             Host, Port, Socket,
                                             Options, User, CbInfo),
    #state{static_env = #static_env{user_socket = UserSocket}} = State0,
    User ! {self(), user_socket, UserSocket},
    try
	State = xssl_gen_statem:init_ssl_config(State0#state.ssl_options,
                                          ?CLIENT_ROLE, State0),
...
terminate(Reason, StateName, State) ->
    xssl_gen_statem:terminate(Reason, StateName, State).
...
initial_hello(Type, Event, State) ->
    xtls_client_connection:initial_hello(Type, Event, State).
...
config_error(Type, Event, State) ->
    xssl_gen_statem:config_error(Type, Event, State).
...
user_hello({call, From}, cancel, State) ->
    ...
    xssl_gen_statem:handle_own_alert(?ALERT_REC(?FATAL, ?USER_CANCELED,
                                               user_canceled),
                                    user_hello, State);
user_hello({call, From}, {handshake_continue, NewOptions, Timeout},
           #state{handshake_env =  #handshake_env{continue_status = pause} = HSEnv,
                  ssl_options = Options0} = State0) ->
    try xssl:update_options(NewOptions, ?CLIENT_ROLE, Options0) of
        Options ->
            State = xssl_gen_statem:ssl_config(Options, ?CLIENT_ROLE, State0),
        ...
        throw:{error, Reason} ->
            ...
            xssl_gen_statem:handle_own_alert(?ALERT_REC(?FATAL, ?INTERNAL_ERROR, Reason),
                                            user_hello, State0)
    end;
user_hello(Type, Msg, State) ->
    xtls_gen_connection_1_3:user_hello(Type, Msg, State).
...
start(enter, _, State0) ->
    State = xtls_gen_connection_1_3:handle_middlebox(State0),
    {next_state, ?STATE(start), State,[]};
start(internal = Type, #change_cipher_spec{} = Msg, State) ->
   xtls_gen_connection_1_3:handle_change_cipher_spec(Type, Msg,
                                                    ?STATE(start), State);
start(internal,
    ...
    case xtls_record:is_acceptable_version(Version, SupportedVersions) of
        true ->
            handle_exlusive_1_3_hello_or_hello_retry_request(ServerHello, State);
        false ->
            xssl_gen_statem:handle_own_alert(
              ?ALERT_REC(?FATAL, ?PROTOCOL_VERSION), ?STATE(start), State)
    end;
...
wait_sh(internal, #server_hello{session_id = ?EMPTY_ID} = Hello,
    ...
    case handle_server_hello(Hello, State0) of
         #alert{} = Alert ->
            xssl_gen_statem:handle_own_alert(Alert, ?STATE(wait_sh), State0);
        {State1, start, ServerHello} ->
            %% hello_retry_request: go to start
             {next_state, start, State1, [{next_event, internal, ServerHello}]};
        {State1, wait_ee} ->
             xtls_gen_connection:next_event(wait_ee, no_record, State1)
    end;
  ...
  when not is_map_key(middlebox_comp_mode, SSLOpts) ->
    ...
    case handle_server_hello(Hello, State0) of
        ...
        {State1, wait_ee} when IsRetry == false ->
            xtls_gen_connection:next_event(hello_middlebox_assert,
                                          no_record, State1)
    end;
wait_sh(info, Msg, State) ->
    xtls_gen_connection:handle_info(Msg, ?STATE(wait_sh), State);
wait_sh(Type, Msg, State) ->
    xssl_gen_statem:handle_common_event(Type, Msg, ?STATE(wait_sh), State).
...
hello_middlebox_assert(info, Msg, State) ->
    xtls_gen_connection:handle_info(Msg, ?STATE(hello_middlebox_assert), State);
hello_middlebox_assert(Type, Msg, State) ->
    xssl_gen_statem:handle_common_event(Type, Msg, ?STATE(hello_middlebox_assert), State).
...
hello_retry_middlebox_assert(info, Msg, State) ->
    xtls_gen_connection:handle_info(Msg, ?STATE(hello_retry_middlebox_assert), State);
...
wait_ee(enter, _, State0) ->
    State = xtls_gen_connection_1_3:handle_middlebox(State0),
    {next_state, ?STATE(wait_ee), State,[]};
...
wait_ee(internal, #encrypted_extensions{extensions = Extensions}, State0) ->
    case handle_encrypted_extensions(Extensions, State0) of
        #alert{} = Alert ->
            xssl_gen_statem:handle_own_alert(Alert, wait_ee, State0);
        {State, NextState} ->
            xtls_gen_connection:next_event(NextState, no_record, State)
    end;
wait_ee(info, Msg, State) ->
    xtls_gen_connection:handle_info(Msg, ?STATE(wait_ee), State);
wait_ee(Type, Msg, State) ->
    xssl_gen_statem:handle_common_event(Type, Msg, ?STATE(wait_ee), State).
...
wait_cert_cr(enter, _, State0) ->
    State = xtls_gen_connection_1_3:handle_middlebox(State0),
    {next_state, ?STATE(wait_cert_cr), State,[]};
...
wait_cert_cr(internal, #certificate_1_3{} = Certificate, State0) ->
    case handle_certificate(Certificate, State0) of
        {#alert{} = Alert, State} ->
            xssl_gen_statem:handle_own_alert(Alert, ?STATE(wait_cert_cr), State);
        {State1, NextState} ->
            xtls_gen_connection:next_event(NextState, no_record, State1)
    end;
wait_cert_cr(internal, #certificate_request_1_3{} = CertificateRequest,
    ...
    case handle_certificate_request(CertificateRequest, State0) of
        #alert{} = Alert ->
            xssl_gen_statem:handle_own_alert(Alert, ?STATE(wait_cert_cr), State0);
        {State1, NextState} ->
            xtls_gen_connection:next_event(NextState, no_record, State1)
   end;
wait_cert_cr(info, Msg, State) ->
    xtls_gen_connection:handle_info(Msg, ?STATE(wait_cert_cr), State);
wait_cert_cr(Type, Msg, State) ->
    xssl_gen_statem:handle_common_event(Type, Msg, ?STATE(wait_cert_cr), State).
...
wait_cert(Type, Msg, State) ->
    xtls_gen_connection_1_3:wait_cert(Type, Msg, State).
...
wait_cv(internal,
    ...
    try
        ...
        {Ref, {#alert{} = Alert, AState}} ->
            xssl_gen_statem:handle_own_alert(Alert, ?STATE(wait_cv), AState)
    end;
wait_cv(Type, Msg, State) ->
    xtls_gen_connection_1_3:wait_cv(Type, Msg, State).
...
wait_finished(internal,
    ...
    try
        ...
        Finished = xtls_handshake_1_3:finished(State3),
        %% Encode Finished
        State4 = Connection:queue_handshake(Finished, State3),
        %% Send first flight
        {State5, _} = Connection:send_handshake_flight(State4),
        State6 = xtls_handshake_1_3:calculate_traffic_secrets(State5),
        State7 =
            xtls_handshake_1_3:maybe_calculate_resumption_master_secret(State6),
        ...
        xtls_gen_connection:next_event(connection, Record,
        ...
        {Ref, #alert{} = Alert} ->
            xssl_gen_statem:handle_own_alert(Alert, wait_finished, State0)
    end;
wait_finished(info, Msg, State) ->
    xtls_gen_connection:handle_info(Msg, ?STATE(wait_finished), State);
wait_finished(Type, Msg, State) ->
    xssl_gen_statem:handle_common_event(Type, Msg, ?STATE(wait_finished), State).
%%--------------------------------------------------------------------
...
connection(Type, Msg, State) ->
    xtls_gen_connection_1_3:connection(Type, Msg, State).
...
downgrade(Type, Msg, State) ->
    xtls_gen_connection_1_3:downgrade(Type, Msg, State).
...
maybe_send_early_data(#state{
                         ...
                         ssl_options = #{versions := [?TLS_1_3|_],
                                         ...
                                         EarlyData =/= undefined ->
    %% D.4.  Middlebox Compatibility Mode
    State1 = xtls_gen_connection_1_3:maybe_queue_change_cipher_spec(State0, last),
    %% Early traffic secret
    EarlyDataSize = xtls_handshake_1_3:early_data_size(EarlyData),
    case xtls_handshake_1_3:get_pre_shared_key_early_data(SessionTickets,
                                                         UseTicket) of
        {ok, {PSK, Cipher, HKDF, MaxSize}} when EarlyDataSize =< MaxSize ->
            State2 =
                xtls_handshake_1_3:calculate_client_early_traffic_secret(Hist,
                                                                        PSK,
                                                                        Cipher,
                                                                        HKDF,
                                                                        State1),
...
maybe_automatic_session_resumption(#state{ssl_options =
  ...
  when ?TLS_GTE(Version, ?TLS_1_3) andalso
    ...
    EarlyDataSize = xtls_handshake_1_3:early_data_size(EarlyData),
    KeyPair =
        xtls_client_ticket_store:find_ticket(self(), Ciphers, HashAlgos,
                                            SNI, EarlyDataSize),
    UseTicket = xtls_handshake_1_3:choose_ticket(KeyPair, EarlyData),
    xtls_client_ticket_store:lock_tickets(self(), [UseTicket]),
...
maybe_generate_client_shares(#{versions := [?TLS_1_3|_],
    ...
    xssl_cipher:generate_client_shares([Group]);
...
handle_exlusive_1_3_hello_or_hello_retry_request(ServerHello, State0) ->
    case do_handle_exlusive_1_3_hello_or_hello_retry_request(ServerHello,
                                                             State0) of
        #alert{} = Alert ->
            xssl_gen_statem:handle_own_alert(Alert, start, State0);
        {State, NextState} ->
...
do_handle_exlusive_1_3_hello_or_hello_retry_request(
  ...
  #state{static_env = #static_env{host = Host,
    ...
    {Ref,Maybe} = xtls_gen_connection_1_3:do_maybe(),
        ...
        ClientKeyShare = xssl_cipher:generate_client_shares([SelectedGroup]),
        TicketData =
            xtls_handshake_1_3:get_ticket_data(self(), SessionTickets, UseTicket),
        OcspNonce = maps:get(ocsp_nonce, StaplingState, undefined),
        Hello0 = xtls_handshake:client_hello(Host, Port,
        ...
        Hello1 = xtls_handshake_1_3:maybe_add_cookie_extension(Cookie, Hello0),

        %% Update state
        State1 =
            xtls_handshake_1_3:update_start_state(State0,
        ...
        State2 = xtls_handshake_1_3:replace_ch1_with_message_hash(State1),
        #state{handshake_env =
                   #handshake_env{tls_handshake_history = HHistory0}} = State2,

        %% Update pre_shared_key extension with binders (TLS 1.3)
        Hello =
            xtls_handshake_1_3:maybe_add_binders(Hello1, HHistory0,
        ...
        {#state{handshake_env = HsEnv} = State3, BinMsg} =
            xtls_gen_connection_1_3:maybe_prepend_change_cipher_spec(State2,
                                                                    BinMsg0),
...
handle_server_hello(#server_hello{cipher_suite = SelectedCipherSuite,
    ...
    try
        ...
        %% Update state
        State2 = xtls_handshake_1_3:update_start_state(State1,
                                    #{cipher => SelectedCipherSuite,
                                     key_share => ClientKeyShare,
                                     session_id => SessionId,
                                     group => SelectedGroup,
        ...
        PSK = Maybe(tls_handshake_1_3:get_pre_shared_key(SessionTickets,
                                                         ...
                                                         ServerPreSharedKey)),
        State3 =
            xtls_handshake_1_3:calculate_handshake_secrets(ServerPublicKey,
                                                          ClientPrivateKey,
                                                          SelectedGroup,
                                                          PSK, State2),
