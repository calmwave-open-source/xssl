...
%% Help functions for xtls|dtls*_connection.erl
...
%%--------------------------------------------------------------------
-spec renegotiation(pid()) -> ok | {error, xssl:reason()}.
%%
%% Description: Starts a renegotiation of the ssl session.
%%--------------------------------------------------------------------
renegotiation(ConnectionPid) ->
    xssl_gen_statem:call(ConnectionPid, renegotiate).

renegotiation(Pid, WriteState) ->
    xssl_gen_statem:call(Pid, {user_renegotiate, WriteState}).
...
%%====================================================================
%% Help functions for xtls|dtls_connection.erl
%%====================================================================


%% Only called by TLS xtls_gen_server and xtls_gen_client (no xdtls)
...
hello(Type, Event, State) ->
    xssl_gen_statem:handle_common_event(Type, Event, ?STATE(hello), State).
...
user_hello({call, From}, {handshake_continue, NewOptions, Timeout},
    ...
    try xssl:update_options(NewOptions, Role, Options0) of
        Options ->
            State = xssl_gen_statem:ssl_config(Options, Role, State0),
            {next_state, hello,
...
abbreviated(Type, Event, State) ->
    xssl_gen_statem:handle_common_event(Type, Event, ?STATE(abbreviated), State).
...
certify(Type, Event, State) ->
    xssl_gen_statem:handle_common_event(Type, Event, certify, State).
...
cipher(Type, Event, State) ->
    xssl_gen_statem:handle_common_event(Type, Event, ?STATE(cipher), State).
...
connection({call, From}, negotiated_protocol,
    ...
    xssl_gen_statem:hibernate_after(connection, State, [{reply, From, {error, protocol_not_negotiated}}]);
connection({call, From}, negotiated_protocol,
	   #state{handshake_env = #handshake_env{alpn = undefined,
                                                 negotiated_protocol = SelectedProtocol}} = State) ->
    xssl_gen_statem:hibernate_after(connection, State,
...
handle_call({export_key_materials, [Label], [Context0], [WantedLength], _}, From, _,
    ...
    Reply = try
                {ok, xtls_v1:prf(PRFAlgorithm, MasterSecret, Label, Seed, WantedLength)}
	    catch
		exit:Reason:ST ->
...
handle_call(Msg, From, StateName, State) ->
   xssl_gen_statem:handle_call(Msg, From, StateName, State).
