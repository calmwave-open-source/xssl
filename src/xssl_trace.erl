    ...
    [{api,
     ...
     {csp, %% OCSP
      ...
      [{ssl_handshake, [{maybe_add_certificate_status_request, 4},
       ...
       {tls_dtls_client_connection, [{wait_stapling, 3}, {certify, 3}]},
       {tls_handshake, [{ocsp_nonce, 1}, {client_hello, 11}]},
       {tls_handshake_1_3, [{validate_certificate_chain, 8},
     ...
     {crt, %% certificates
      ...
      [{public_key, [{pkix_path_validation, 3}, {path_validation, 2},
       ...
       {tls_client_connection, [{init,1}]},
       {tls_server_connection, [{init,1}]},
       {dtls_client_connection, [{init,1}]},
       {dtls_server_connection, [{init,1}]}]},
     {kdt, %% key update
      fun(M, F, A) -> dbg:tpl(M, F, A, x) end,
      fun(M, F, A) -> dbg:ctpl(M, F, A) end,
