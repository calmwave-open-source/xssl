    ...
    [{api,
     ...
     {csp, %% OCSP
      ...
      [{xssl_handshake, [{maybe_add_certificate_status_request, 4},
       ...
       {xssl, [{opt_stapling, 3}]},
       {xssl_certificate, [{verify_cert_extensions, 5}]},
       {xssl_test_lib, [{init_openssl_server, 3}, {openssl_server_loop, 3}]},
       {xtls_client_connection, [{wait_stapling, 3}]},
       {xdtls_client_connection, [{initial_hello, 3}, {hello, 3}, {connection, 3}]},
       {xtls_dtls_client_connection, [{wait_stapling, 3}, {certify, 3}]},
       {xtls_handshake, [{ocsp_nonce, 1}, {client_hello, 11}]},
       {xtls_handshake_1_3, [{validate_certificate_chain, 8},
                            {process_certificate, 2}]},
       {xdtls_handshake, [{client_hello, 8}]}]},
     {crt, %% certificates
      fun(M, F, A) -> dbg:tpl(M, F, A, x) end,
      fun(M, F, A) -> dbg:ctpl(M, F, A) end,
      [{public_key, [{pkix_path_validation, 3}, {path_validation, 2},
       ...
       {xssl_pkix_db, [{decode_cert, 2}]},
       {xtls_handshake_1_3, [{path_validation, 10}]},
       {xtls_server_connection_1_3, [{init,1}]},
       {xtls_client_connection_1_3, [{init,1}]},
       {xtls_client_connection, [{init,1}]},
       {xtls_server_connection, [{init,1}]},
       {xdtls_client_connection, [{init,1}]},
       {xdtls_server_connection, [{init,1}]}]},
     {kdt, %% key update
      fun(M, F, A) -> dbg:tpl(M, F, A, x) end,
      fun(M, F, A) -> dbg:ctpl(M, F, A) end,
      [{xtls_gen_connection_1_3, [{handle_key_update, 2}]},
       {xtls_sender, [{init, 3}, {time_to_rekey, 6},
                     {send_post_handshake_data, 4}]},
       {xtls_v1, [{update_traffic_secret, 2}]}]},
     {rle, %% role
      fun(M, F, A) -> dbg:tpl(M, F, A, x) end,
      fun(M, F, A) -> dbg:ctpl(M, F, A) end,
      [{xssl, [{listen,2}, {connect,3}]},
       {xssl_gen_statem, [{init, 1}]},
       {xtls_server_session_ticket, [{init,1}]},
       {xtls_sender, [{init, 3}]}]},
     {ssn, %% session
      fun(M, F, A) -> dbg:tpl(M, F, A, x) end,
      fun(M, F, A) -> dbg:ctpl(M, F, A) end,
