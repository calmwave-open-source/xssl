...
initial_state(Role, Tab, Host, Port, Socket,
    ...
    ConnectionStates = xdtls_record:init_connection_states(Role, BeastMitigation),
    #{session_cb := SessionCacheCb} = xssl_config:pre_1_3_session_opts(Role),
    InternalActiveN = xssl_config:get_internal_active_n(),
    Monitor = erlang:monitor(process, User),

    SslSocket = xdtls_socket:socket([self()], CbModule, Socket, ?MODULE, Tab),

    InitStatEnv = #static_env{
                     user_socket = SslSocket,
                     role = Role,
                     transport_cb = CbModule,
                     protocol_cb = xdtls_gen_connection,
                     data_tag = DataTag,
                     close_tag = CloseTag,
                     error_tag = ErrorTag,
...
start_fsm(Role, Host, Port, Socket, Opts, User, CbInfo, Timeout) ->
    try
	{ok, Pid} = xdtls_connection_sup:start_child([Role, Host, Port, Socket,
						     Opts, User, CbInfo]),
        receive {Pid, user_socket, SslSocket} ->
                {ok, SslSocket} = xssl_gen_statem:socket_control(SslSocket),
...
next_record(#state{protocol_buffers =
    ...
    CurrentRead = xdtls_record:get_connection_state_by_epoch(Epoch, ConnectionStates, read),
    case is_reliable(DataTag) of
        true ->
            decode_cipher_text(State) ;
        false ->
            case xdtls_record:replay_detect(CT, CurrentRead) of
                false ->
                    decode_cipher_text(State);
                true ->
...
next_record(#state{static_env = #static_env{role = server,
    ...
    xdtls_packet_demux:active_once(Listener, Client, self()),
    {no_record, State};
next_record(#state{protocol_specific = #{active_n_toggle := true,
    ...
    case xdtls_socket:setopts(Transport, Socket, [{active,N}]) of
        ok ->
...
handle_protocol_record(#ssl_tls{type = ?APPLICATION_DATA, fragment = Data}, StateName0, State0) ->
	...
	{Record, State1} ->
            case next_event(StateName0, Record, State1) of
                {next_state, StateName, State} ->
                    xssl_gen_statem:hibernate_after(StateName, State, []);
...
handle_protocol_record(#ssl_tls{type = ?HANDSHAKE, epoch = Epoch, fragment = Data},
    ...
    try
	case xdtls_handshake:get_dtls_handshake(Version, Data, Buffers0, Options) of
	    {[], Buffers} ->
...
prepare_flight(#state{handshake_env = #handshake_env{flight_buffer = Flight} = HsEnv,
    ...
    ConnectionStates = xdtls_record:save_current_connection_state(ConnectionStates0, write),
...
queue_change_cipher(ChangeCipher, #state{handshake_env =
    ...
    ConnectionStates = xdtls_record:next_epoch(ConnectionStates0, write),
...
encode_alert(#alert{} = Alert, Version, ConnectionStates) ->
    xdtls_record:encode_alert_record(Alert, Version, ConnectionStates).
...
close(_, Socket, Transport, _) ->
    xdtls_socket:close(Transport,Socket).
...
send_application_data(Data, From, StateName,
	...
	false ->
	    {Msgs, ConnectionStates} =
                xdtls_record:encode_data(Data, Version, ConnectionStates0),
...
send(Transport, {Listener, Socket}, Data) when is_pid(Listener) -> 
    %% Server socket
    xdtls_socket:send(Transport, Socket, Data);
...
setopts(Transport, Socket, Other) ->
    xdtls_socket:setopts(Transport, Socket, Other).

getopts(Transport, Socket, Tag) ->
    xdtls_socket:getopts(Transport, Socket, Tag).
...
handle_info({socket_reused, Client}, StateName,
    ...
    Alert = ?ALERT_REC(?FATAL, ?CLOSE_NOTIFY, transport_closed),
    xssl_gen_statem:handle_normal_shutdown(Alert#alert{role = server}, StateName, State),
    {stop, {shutdown, transport_closed}, State};
...
encode_handshake_flight(Flight, Version, MaxFragmentSize, Epoch, ConnectionStates) ->
    Encode = fun(Fragment, {Acc, Cs0}) ->
                     {Enc, Cs} = xdtls_record:encode_handshake(Fragment, Version, Epoch, Cs0),
                     {[Enc|Acc], Cs}
             end,
    {Rev, Cs} = lists:foldl(fun(Handshake, {Acc,Cs0}) ->
                                    Frags = xdtls_handshake:fragment_handshake(Handshake, MaxFragmentSize),
...
encode_change_cipher(#change_cipher_spec{}, Version, Epoch, ConnectionStates) ->
    xdtls_record:encode_change_cipher_spec(Version, Epoch, ConnectionStates).
...
decode_cipher_text(#state{protocol_buffers = #protocol_buffers{dtls_cipher_texts = [ CT | Rest]} = Buffers,
    ...
    case xdtls_record:decode_cipher_text(CT, ConnStates0) of
	{Plain, ConnStates} ->		      
