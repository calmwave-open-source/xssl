...
-module(xtls_server_session_ticket).
-moduledoc false.
...
handle_call({new_session_ticket, Prf, MasterSecret, PeerCert}, _From,
    ...
    PSK = xtls_v1:pre_shared_key(MasterSecret, ticket_nonce(Nonce), Prf),
...
handle_info(rotate_bloom_filters, 
    ...
    BloomFilter = xtls_bloom_filter:rotate(BloomFilter0),
...
initial_state([stateless, Lifetime, _, MaxEarlyDataSize, {Window, K, M}, Seed]) ->
    erlang:send_after(Window * 1000, self(), rotate_bloom_filters),
    #state{nonce = 0,
           stateless = #{bloom_filter => xtls_bloom_filter:new(K, M),
...
validate_binder(Binder, HandshakeHist, PSK, Prf, AlertDetail) ->
      case xtls_handshake_1_3:is_valid_binder(Binder, HandshakeHist, PSK, Prf) of
          true ->
              true;
          false ->
...
stateless_anti_replay(Index, PSK, Binder, PeerCert,
                      #state{stateless = #{bloom_filter := BloomFilter0} 
                             = Stateless} = State) ->
    case xtls_bloom_filter:contains(BloomFilter0, Binder) of
        true ->
            %%possible_replay
            {{ok, undefined}, State};
        false ->
            BloomFilter = xtls_bloom_filter:add_elem(BloomFilter0, Binder),
            {{ok, {Index, PSK, PeerCert}},
