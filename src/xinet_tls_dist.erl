...
hs_data_ssl(Family, #xsslsocket{payload_sender = DistCtrl} = SslSocket) ->
    {ok, Address} =
        maybe
            {error, einval} ?= xssl:peername(SslSocket),
...
f_send(SslSocket, Packet) ->
    xssl:send(SslSocket, Packet).

f_recv(SslSocket, Length, Timeout) ->
    case xssl:recv(SslSocket, Length, Timeout) of
...
f_setopts_post_nodeup(SslSocket) ->
    xssl:setopts(SslSocket, [inet_tcp_dist:nodelay()]).
...
mf_setopts(SslSocket, Opts) ->
    case setopts_filter(Opts) of
        [] ->
            xssl:setopts(SslSocket, Opts);
        Opts1 ->
            {error, {badopts,Opts1}}
    end.

mf_getopts(SslSocket, Opts) ->
    xssl:getopts(SslSocket, Opts).

f_handshake_complete(DistCtrl, Node, DHandle) ->
    xtls_sender:dist_handshake_complete(DistCtrl, Node, DHandle).
...
accept_one(Family, Socket, NetKernel) ->
    ...
    of
        {ok, SslSocket} ->
            Receiver = SslSocket#xsslsocket.connection_handler,
            case KTLS of
                true ->
                    {ok, KtlsInfo} = xssl_gen_statem:ktls_handover(Receiver),
                ...
                false ->
                    accept_one(
                      ...
                      fun xssl:controlling_process/2)
            end;
...
do_accept(
  Family, AcceptPid, DistSocket, MyNode, Allowed, SetupTime, Kernel) ->
	...
	{AcceptPid, controller} ->
            ...
            {HSData0, NewAllowed} =
                case DistSocket of
                    SslSocket = #xsslsocket{payload_sender = Sender} ->
                        link(Sender),
...
allowed_nodes(_SslSocket, []) ->
    ...
    [];
allowed_nodes(SslSocket, Allowed) ->
    case xssl:peercert(SslSocket) of
        {ok,PeerCertDER} ->
            case xssl:peername(SslSocket) of
                {ok,{PeerIP,_Port}} ->
                    PeerCert =
...
do_setup(
  Family, Node, Type, MyNode, LongOrShortNames, SetupTime, NetKernel) ->
    ...
    maybe
        {ok, #xsslsocket{connection_handler = Receiver, payload_sender = Sender} = SslSocket} ?=
            xssl:connect(Ip, PortNum, Opts, net_kernel:connecttime()),
        HSData =
            case KTLS of
                true ->
                    {ok, KtlsInfo} =
                        xssl_gen_statem:ktls_handover(Receiver),
                ...
                false ->
                    ...
                    ok = xssl:controlling_process(SslSocket, self()),
                    link(Sender),
                    hs_data_ssl(Family, SslSocket)
            end
            #hs_data{
