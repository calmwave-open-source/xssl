...
-module(xtls_gen_connection_1_3).
-moduledoc false.
...
initial_state(Role, Sender, Tab, Host, Port, Socket,
    ...
    true = ets:insert(Tab, {{socket_options, packet}, SocketOptions#xsocket_options.packet}),

    InitStatEnv = #static_env{
                     role = Role,
                     user_socket = SslSocket,
                     transport_cb = CbModule,
                     protocol_cb = xtls_gen_connection,
                     data_tag = DataTag,
                     close_tag = CloseTag,
                     error_tag = ErrorTag,
...
user_hello(info, {'DOWN', _, _, _, _} = Event, State) ->
    xssl_gen_statem:handle_info(Event, ?STATE(user_hello), State);
user_hello(_, _, _) ->
...
wait_cert(internal,
    ...
    case do_wait_cert(Certificate, State0) of
        {#alert{} = Alert, State} ->
            xssl_gen_statem:handle_own_alert(Alert, ?STATE(wait_cert), State);
        {State, NextState} ->
            xtls_gen_connection:next_event(NextState, no_record, State)
    end;
wait_cert(info, Msg, State) ->
    xtls_gen_connection:handle_info(Msg, ?STATE(wait_cert), State);
wait_cert(Type, Msg, State) ->
    xssl_gen_statem:handle_common_event(Type, Msg, ?STATE(wait_cert), State).
...
wait_cv(info, Msg, State) ->
    xtls_gen_connection:handle_info(Msg, ?STATE(wait_cv), State);
wait_cv(Type, Msg, State) ->
    xssl_gen_statem:handle_common_event(Type, Msg, ?STATE(wait_cv), State).
...
connection(internal, #new_session_ticket{} = NewSessionTicket, State) ->
    ...
    xtls_gen_connection:next_event(?STATE(connection), no_record, State);
...
connection(internal, #key_update{} = KeyUpdate, State0) ->
    case handle_key_update(KeyUpdate, State0) of
        {ok, State} ->
            xtls_gen_connection:next_event(?STATE(connection), no_record, State);
        {error, State, Alert} ->
            xssl_gen_statem:handle_own_alert(Alert, ?STATE(connection), State),
            xtls_gen_connection:next_event(?STATE(connection), no_record, State)
    end;
connection({call, From}, negotiated_protocol,
	   #state{handshake_env = #handshake_env{alpn = undefined}} = State) ->
    xssl_gen_statem:hibernate_after(?STATE(connection), State,
...
connection(Type, Event, State) ->
    xssl_gen_statem:connection(Type, Event, State).
...
handle_change_cipher_spec(Type, Msg, StateName,
    ...
    case maps:get(change_cipher_spec, PS0) of
        ignore ->
            ...
            xtls_gen_connection:next_event(StateName, no_record,
                                          State#state{protocol_specific = PS});
        fail ->
            xssl_gen_statem:handle_common_event(Type, Msg, StateName, State)
    end.
...
maybe_prepend_change_cipher_spec(#state{
    ...
    CCSBin = xtls_handshake_1_3:create_change_cipher_spec(State),
...
send_key_update(Sender, Type) ->
    KeyUpdate = xtls_handshake_1_3:key_update(Type),
    xtls_sender:send_post_handshake(Sender, KeyUpdate).
...
update_cipher_key(ConnStateName, CS0) ->
    ...
    ApplicationTrafficSecret =
        xtls_v1:update_traffic_secret(HKDF,
                                     ApplicationTrafficSecret0),

    %% Calculate traffic keys
    KeyLength = xtls_v1:key_length(CipherSuite),
    {Key, IV} = xtls_v1:calculate_traffic_keys(HKDF, KeyLength,
                                              ApplicationTrafficSecret),
...
handle_new_session_ticket(#new_session_ticket{ticket_nonce = Nonce}
  ...
  when SessionTickets =:= manual ->
    ...
    PSK = xtls_v1:pre_shared_key(RMS, Nonce, HKDF),
...
handle_new_session_ticket(#new_session_ticket{ticket_nonce = Nonce}
  ...
  when SessionTickets =:= auto ->
    ...
    PSK = xtls_v1:pre_shared_key(RMS, Nonce, HKDF),
    SNI = maps:get(server_name_indication, SslOpts, undefined),
    xtls_client_ticket_store:store_ticket(NewSessionTicket, {Cipher, HKDF}, SNI, PSK).
...
exporter_secrets(ExporterMasterSecret, Labels, PRFAlgorithm) ->
    DeriveSecret =
        fun(Label) ->
                xtls_v1:derive_secret(ExporterMasterSecret, Label, <<>>, PRFAlgorithm)
        end,
...
exporter(ExporterSecret, Context0, WantedLength, PRFAlgorithm) ->
    ...
    HashContext = xtls_v1:transcript_hash(Context, PRFAlgorithm),
    xtls_v1:hkdf_expand_label(ExporterSecret, <<"exporter">>, HashContext,
