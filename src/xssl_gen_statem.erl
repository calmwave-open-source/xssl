...
%%--------------------------------------------------------------------
%%--------------------------------------------------------------------
-spec start_link(client| server, pid(), xssl:host(), inet:port_number(), port(), tuple(), pid(), tuple()) ->
    {ok, pid()} | ignore |  {error, xssl:reason()}.
%%
...
%%--------------------------------------------------------------------
-spec start_link(atom(), xssl:host(), inet:port_number(), port(), tuple(), pid(), tuple()) ->
			{ok, pid()} | ignore |  {error, xssl:reason()}.
%%
...
init([Role, Sender |[Host, Port, _Socket, {TLSOpts, _, _}, _User, _CbInfo] = InitArgs]) ->
    ...
    case Role of
        ?CLIENT_ROLE ->
            case TLSOpts of
                #{versions := [?TLS_1_3]} ->
                    xtls_client_connection_1_3:init([Role, Sender, Tab|InitArgs]);
                _  ->
                    xtls_client_connection:init([Role, Sender, Tab | InitArgs])
            end;
        ?SERVER_ROLE ->
            case TLSOpts of
                #{versions := [?TLS_1_3]} ->
                    xtls_server_connection_1_3:init([Role, Sender, Tab|InitArgs]);
                _ ->
                    xtls_server_connection:init([Role, Sender, Tab|InitArgs])
            end
    end;
init([Role | [Host, Port, _Socket, {DTLSOpts,_,_}, _User, _CbInfo] = InitArgs]) ->
    ...
    case Role of
        ?CLIENT_ROLE ->
            xdtls_client_connection:init([Role, Tab|InitArgs]);
        ?SERVER_ROLE ->
            xdtls_server_connection:init([Role, Tab|InitArgs])
    end.
...
-spec connect(xtls_gen_connection | xdtls_gen_connection,
	      ...
	      {ssl_options(), #xsocket_options{},
	       ...
	       undefined},
	      pid(), tuple(), timeout()) ->
		     {ok, #xsslsocket{}} | {error, xssl:reason()}.
%%
...
%%--------------------------------------------------------------------
-spec handshake(#xsslsocket{}, timeout()) ->  {ok, #xsslsocket{}} |
                                             {ok,  #xsslsocket{}, map()}| {error, xssl:reason()}.
%%
%% Description: Starts ssl handshake.
%%--------------------------------------------------------------------
handshake(#xsslsocket{connection_handler = Pid} = Socket, Timeout) ->
...
%%--------------------------------------------------------------------
-spec handshake(#xsslsocket{}, {SSLOpts::list(), #xsocket_options{}}, timeout()) ->
          {ok, #xsslsocket{}} | {ok, #xsslsocket{}, map()} | {error, xssl:reason()}.
%%
%% Description: Starts ssl handshake with some new options
%%--------------------------------------------------------------------
handshake(#xsslsocket{connection_handler = Pid} = Socket, SslOptions, Timeout) ->
...
%%--------------------------------------------------------------------
-spec handshake_continue(#xsslsocket{}, [xssl:tls_server_option()],
                         timeout()) ->  {ok,  #xsslsocket{}}| {error, xssl:reason()}.
%%
%% Description: Continues handshake with new options
%%--------------------------------------------------------------------
handshake_continue(#xsslsocket{connection_handler = Pid} = Socket, SslOptions, Timeout) ->
...
%%--------------------------------------------------------------------
-spec handshake_cancel(#xsslsocket{}) ->  ok | {error, xssl:reason()}.
%%
%% Description: Cancels connection
%%--------------------------------------------------------------------
handshake_cancel(#xsslsocket{connection_handler = Pid}) ->
...
-spec socket_control(#xsslsocket{}) ->
          {ok, #xsslsocket{}} | {error, xssl:reason()}.

socket_control(#xsslsocket{connection_cb = xdtls_gen_connection,
                          listener_config = Config} = SslSocket)
  when Config =/= undefined ->
    %% xdtls listener process must have the socket control
    {ok, SslSocket};
socket_control(#xsslsocket{connection_cb = xdtls_gen_connection,
...
socket_control(#xsslsocket{transport_cb = Transport,
...
%%--------------------------------------------------------------------
-spec send(pid(), iodata()) -> ok | {error, xssl:reason()}.
%%
...
-spec recv(pid(), integer(), timeout()) ->
    {ok, binary() | list()} | {error, xssl:reason()}.
%%
...
%%--------------------------------------------------------------------
-spec connection_information(pid(), boolean()) -> {ok, list()} | {error, xssl:reason()}.
%%
...
%%--------------------------------------------------------------------
-spec shutdown(pid(), atom()) -> ok | {error, xssl:reason()}.
%%
...
%%--------------------------------------------------------------------
-spec new_user(pid(), pid()) ->  ok | {error, xssl:reason()}.
%%
...
%%--------------------------------------------------------------------
-spec get_opts(pid(), list()) -> {ok, list()} | {error, xssl:reason()}.
%%
...
%%--------------------------------------------------------------------
-spec set_opts(pid(), list()) -> ok | {error, xssl:reason()}.
%%
...
%%--------------------------------------------------------------------
-spec peer_certificate(pid()) -> {ok, binary()| undefined} | {error, xssl:reason()}.
%%
...
%%--------------------------------------------------------------------
-spec negotiated_protocol(pid()) -> {ok, binary()} | {error, xssl:reason()}.
%%
...
%%--------------------------------------------------------------------
-spec ktls_handover(pid()) -> {ok, map()} | {error, xssl:reason()}.
%%
%% Description:  Returns the negotiated protocol
%%--------------------------------------------------------------------
...
connection({call, From}, 
    ...
    case xtls_sender:downgrade(Sender, Timeout) of
        {ok, Write} ->
            ...
            %% more may be sent by the xtls_sender process.
...
connection(cast, {dist_handshake_complete, DHandle},
    ...
    State1 =
        State0#state{
          socket_options = SockOpts#xsocket_options{active = true},
...
downgrade(internal, #alert{description = ?CLOSE_NOTIFY},
    ...
    xtls_socket:setopts(Transport, Socket, [{active, false}, {packet, 0}, {mode, binary}]),
...
handle_call({recv, _N, _Timeout}, From, _,
		  #state{socket_options =
			     #xsocket_options{active = Active}}) when Active =/= false ->
...
handle_call({set_opts, Opts0}, From, StateName,
    ...
    case {proplists:lookup(active, Opts0), Opts} of
        {{_, N}, #xsocket_options{active=false}} when is_integer(N) ->
            send_user(Pid,{ssl_passive,UserSocket});
        _ ->
            ok
    end,
    State = State0#state{socket_options = Opts},
    handle_active_option(Opts#xsocket_options.active, StateName, From, Reply, State);
...
terminate(_, _, #state{connection_env = #connection_env{socket_terminated = true}}) ->
    %% Happens when user closes the connection using xssl:close/1 e.i. terminate
    %% is called explicitly beforehand to to guarantee that Transport:close has been called
    %% when xssl:close/1 returns. Or when using xssl:shutdown/2 with read_write
    ok;
...
update_ssl_options_from_sni(#{sni_fun := SNIFun} = OrigSSLOptions, SNIHostname) ->
    case SNIFun(SNIHostname) of
        ...
        SSLOptions ->
            ...
            xssl:update_options(SSLOptions, server, FallBackOptions)
    end.
...
read_application_data_deliver(State, Front, BufferSize, Rear, SocketOpts0, RecvFrom, Data) ->
    ...
    if
        SocketOpts#xsocket_options.active =:= false ->
...
%% Picks ClientData
get_data(#xsocket_options{active=false}, undefined, _Bin) ->
    %% Recv timed out save buffer data until next recv
    passive;
get_data(#xsocket_options{active=Active, packet=Raw}, BytesToRead, Bin)
...
get_data(#xsocket_options{packet=Type, packet_size=Size}, _, Bin) ->
...
%%    #xsocket_options.packet =:= http: Expect a HTTP Request/Response line
%%    #xsocket_options.packet =:= {http, headers}: Expect HTTP Headers
%% Note that if the user has explicitly configured the socket to expect
%% HTTP headers using the {packet, httph} option, we don't do any automatic
%% switching of states.
deliver_app_data(UserSocket, #xsocket_options{active=Active, packet=Type} = SOpts,
                 Data, Pid, From) ->
        ...
        case Data of
            {P, _, _, _}
              when ((P =:= http_request) or (P =:= http_response)),
                ...
                SOpts#xsocket_options{packet={Type, headers}};
            http_eoh when tuple_size(Type) =:= 2 ->
                %% End of headers - expect another Request/Response line
                {Type1, headers} = Type,
                SOpts#xsocket_options{packet=Type1};
            _ ->
                SOpts
        end,
    case Active of
        once ->
            SO#xsocket_options{active=false};
        1 ->
            send_user(Pid, {ssl_passive, UserSocket}),
            SO#xsocket_options{active=false};
        N when is_integer(N) ->
            SO#xsocket_options{active=N - 1};
	_ ->
	    SO
    end.

format_reply(_UserSocket, #xsocket_options{active = false, mode = Mode, packet = Packet,
                                          header = Header}, Data) ->
    {ok, do_format_reply(Mode, Packet, Header, Data)};
format_reply(UserSocket, #xsocket_options{active = _, mode = Mode, packet = Packet, header = Header},
             Data) ->
    {ssl, UserSocket, do_format_reply(Mode, Packet, Header, Data)}.

deliver_packet_error(UserSocket, SO= #xsocket_options{active = Active}, Data, Pid, From) ->
    send_or_reply(Active, Pid, From, format_packet_error(UserSocket, SO, Data)).

format_packet_error(_, #xsocket_options{active = false, mode = Mode}, Data) ->
    {error, {invalid_packet, do_format_reply(Mode, raw, 0, Data)}};
format_packet_error(UserSocket, #xsocket_options{active = _, mode = Mode}, Data) ->
...
alert_user(UserSocket, _, Opts, Pid, From, Alert, Role, connection = StateName, Connection) ->
    alert_user(UserSocket, Opts#xsocket_options.active, Pid, From, Alert, Role, StateName, Connection);
...
get_socket_opts(Connection, Transport, Socket, [mode | Tags], SockOpts, Acc) ->
    get_socket_opts(Connection, Transport, Socket, Tags, SockOpts,
		    [{mode, SockOpts#xsocket_options.mode} | Acc]);
get_socket_opts(Connection, Transport, Socket, [packet | Tags], SockOpts, Acc) ->
    case SockOpts#xsocket_options.packet of
	{Type, headers} ->
...
set_socket_opts(ConnectionCb, Transport, Socket, Tab, [{active, Active}| Opts], SockOpts, Other)
  when Active == once; Active == true; Active == false ->
    set_socket_opts(ConnectionCb, Transport, Socket, Tab, Opts,
		    SockOpts#xsocket_options{active = Active}, Other);
set_socket_opts(ConnectionCb, Transport, Socket, Tab, [{active, Active1} = Opt| Opts],
                SockOpts=#xsocket_options{active = Active0}, Other)
...
maybe_add_keylog({_, 'tlsv1.3'}, Info) ->
    try
        ...
        %% is stored in xtls_sender process state
