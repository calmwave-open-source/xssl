...
Example:
```erlang
1> xssl:start(), xssl:connect("google.com", 443, [{verify, verify_peer},
...
-include("xssl_cipher.hrl").
-include("xssl_handshake.hrl").
-include("xssl_srp.hrl").
...
-removed({ssl_accept, '_', 
          "use xssl:handshake/1,2,3 instead"}).
-removed({cipher_suites, 0, 
          "use xssl:cipher_suites/2,3 instead"}).
-removed({cipher_suites, 1, 
          "use xssl:cipher_suites/2,3 instead"}).
-removed([{negotiated_next_protocol,1,
           "use xssl:negotiated_protocol/1 instead"}]).
-removed([{connection_info,1,
           "use xssl:connection_information/1,2 instead"}]).
...
Function [xssl:cipher_suites/2 ](`cipher_suites/2`) can be used to find all
...
filtered for crypto library support using [xssl:filter_cipher_suites/2
](`filter_cipher_suites/2`).

The following function can help creating customized cipher suite lists:

- [xssl:append_cipher_suites/2 ](`append_cipher_suites/2`)
- [xssl:prepend_cipher_suites/2](`prepend_cipher_suites/2`)
- [xssl:suite_to_str/1](`suite_to_str/1`)
- [xssl:str_to_suite/1](`str_to_suite/1`)
- [xssl:suite_to_openssl_str/1](`suite_to_openssl_str/1`)
...
- **`{session_tickets, SessionTickets}`** - Use of session tickets
  ...
  by [application variables](ssl_app.md#xconfiguration).
...
- **`{fallback, LegacyFallback}`** - Inter-op legacy client option
  ...
  > following manner:
  >
  > `xssl:connect(Host, Port, [...{versions, ['tlsv2', 'tlsv1.1', 'tlsv1']}])`
  >
  > `xssl:connect(Host, Port, [...{versions, [tlsv1.1', 'tlsv1']}, {fallback, true}])`
  >
  > `xssl:connect(Host, Port, [...{versions, ['tlsv1']}, {fallback, true}])`
  >
...
- **`{session_tickets, SessionTickets}`**
  ...
  * `stateless_with_cert`
  ...
  exchange involved. Therefore, `xssl:peercert/1` will not return the
...
listen(Port, Options0)
  when is_integer(Port), is_list(Options0) ->
	...
	{ok, Config} = handle_options(Options0, server, undefined),
        do_listen(Port, Config, Config#xconfig.connection_cb)
    catch
	Error = {error, _} ->
	    Error
    end.
...
transport_accept(#xsslsocket{socket_handle = ListenSocket,
...
handshake(#xsslsocket{connection_handler = Controller} = Socket, Timeout)
  when ?IS_TIMEOUT(Timeout), is_pid(Controller) ->
    xssl_gen_statem:handshake(Socket, Timeout);
...
handshake(#xsslsocket{} = Socket, [], Timeout)
  when ?IS_TIMEOUT(Timeout) ->
    handshake(Socket, Timeout);
handshake(#xsslsocket{connection_cb = xtls_gen_connection,
  ...
  when is_list(SslOpts), is_list(Trackers), ?IS_TIMEOUT(Timeout) ->
	...
	{ok, EmOpts, _} = xtls_socket:get_all_opts(Tracker),
	xssl_gen_statem:handshake(Socket, {SslOpts,
					  xtls_socket:emulated_socket_options(EmOpts, #xsocket_options{})}, Timeout)
    catch
	Error = {error, _Reason} -> Error
    end;
handshake(#xsslsocket{socket_handle = {Controller,_}, connection_cb = xdtls_gen_connection} = Socket, SslOpts, Timeout)
  when is_list(SslOpts), ?IS_TIMEOUT(Timeout) ->
    try
        {ok, EmOpts, _} = xdtls_packet_demux:get_all_opts(Controller),
	xssl_gen_statem:handshake(Socket, {SslOpts,
                                          xtls_socket:emulated_socket_options(EmOpts, #xsocket_options{})}, Timeout)
    catch
	Error = {error, _Reason} -> Error
...
handshake(Socket, SslOptions, Timeout)
  when is_list(SslOptions), ?IS_TIMEOUT(Timeout) ->
    try
        ...
        ok = xtls_socket:setopts(Transport, Socket, xtls_socket:internal_inet_values()),
        {ok, Port} = xtls_socket:port(Transport, Socket),
        {ok, SessionIdHandle} = xtls_socket:session_id_tracker(ssl_unknown_listener, SslOpts),
        xssl_gen_statem:handshake(ConnetionCb, Port, Socket,
                                 {SslOpts, 
                                  xtls_socket:emulated_socket_options(EmOpts, #xsocket_options{}),
...
handshake_continue(Socket, SSLOptions, Timeout)
  when is_list(SSLOptions), ?IS_TIMEOUT(Timeout) ->
    xssl_gen_statem:handshake_continue(Socket, SSLOptions, Timeout).

%%--------------------------------------------------------------------
-doc "Cancel the handshake with a fatal `USER_CANCELED` alert.".
-doc(#{title => <<"Client and Server API">>,
       since => <<"OTP 21.0">>}).
-spec  handshake_cancel(#xsslsocket{}) -> any().
%%--------------------------------------------------------------------
handshake_cancel(Socket) ->
    xssl_gen_statem:handshake_cancel(Socket).
...
-spec  close(SslSocket) -> ok | {error, Reason} when
      ...
      Reason :: any().
%%--------------------------------------------------------------------
close(#xsslsocket{listener_config = #xconfig{connection_cb = xdtls_gen_connection}} = DTLSListen) ->
    xdtls_socket:close_listen(DTLSListen, ?DEFAULT_TIMEOUT);
close(#xsslsocket{socket_handle = ListenSocket,
                 listener_config = #xconfig{connection_cb = xtls_gen_connection,
                                           transport_info = Info}}) ->
    Transport = element(1, Info),
    Transport:close(ListenSocket);
close(#xsslsocket{connection_handler = Controller}) when is_pid(Controller) ->
    xssl_gen_statem:close(Controller, {close, ?DEFAULT_TIMEOUT}).
...
%%--------------------------------------------------------------------
close(#xsslsocket{connection_handler = Controller}, {Pid, Timeout} = DownGrade)
  when is_pid(Controller), is_pid(Pid), ?IS_TIMEOUT(Timeout) ->
    case xssl_gen_statem:close(Controller, {close, DownGrade}) of
...
close(#xsslsocket{connection_handler = Controller}, Timeout)
  when is_pid(Controller), ?IS_TIMEOUT(Timeout) ->
    xssl_gen_statem:close(Controller, {close, Timeout});
close(#xsslsocket{listener_config = #xconfig{connection_cb = xdtls_gen_connection}} = DTLSListen, Timeout)
  when ?IS_TIMEOUT(Timeout) ->
    xdtls_socket:close_listen(DTLSListen, Timeout);
close(#xsslsocket{socket_handle = ListenSocket, 
                 listener_config = #xconfig{connection_cb = xtls_gen_connection,
                                           transport_info = Info}}, _) ->
    Transport = element(1, Info),
    xtls_socket:close(Transport, ListenSocket).
...
""".
%%--------------------------------------------------------------------
send(#xsslsocket{payload_sender = Sender,
                connection_cb = xdtls_gen_connection}, Data) when is_pid(Sender) ->
    xssl_gen_statem:send(Sender, Data);
send(#xsslsocket{payload_sender = Sender, tab = Tab,
                connection_cb = xtls_gen_connection}, Data0) when is_pid(Sender) ->
    try
        ...
        case encode_packet(Packet, Data0) of
            {error, _} = Error ->
                Error;
            Data ->
                xtls_sender:send_data(Sender,  erlang:iolist_to_iovec(Data))
        end
    catch error:badarg ->
            {error, closed}
    end;
send(#xsslsocket{listener_config = #xconfig{connection_cb = xdtls_gen_connection}}, _) ->
    {error,enotconn}; %% Emulate connection behaviour
send(#xsslsocket{socket_handle = ListenSocket, 
                listener_config = #xconfig{transport_info = Info}}, Data) ->
    Transport = element(1, Info),
    xtls_socket:send(Transport, ListenSocket, Data). %% {error,enotconn}
...
recv(#xsslsocket{connection_handler = Controller}, Length, Timeout)
  when is_pid(Controller), (is_integer(Length) andalso Length >= 0), ?IS_TIMEOUT(Timeout) ->
    xssl_gen_statem:recv(Controller, Length, Timeout);
recv(#xsslsocket{listener_config = #xconfig{connection_cb = xdtls_gen_connection}}, _, _) ->
    {error,enotconn};
recv(#xsslsocket{socket_handle = Listen,
                listener_config = #xconfig{transport_info = Info}},_,_) ->
    Transport = element(1, Info),
...
%% or once.
%%--------------------------------------------------------------------
controlling_process(#xsslsocket{connection_handler = Controller}, NewController)
  when is_pid(Controller), is_pid(NewController) ->
    xssl_gen_statem:new_user(Controller, NewController);
controlling_process(#xsslsocket{listener_config = #xconfig{connection_cb = xdtls_gen_connection}}, NewController)
  when is_pid(NewController) ->
    ok; %% Meaningless but let it be allowed to conform with TLS 
controlling_process(#xsslsocket{socket_handle = Listen,
                               listener_config = #xconfig{transport_info = Info}},
		    NewController)
  when is_pid(NewController) ->
    Transport = element(1, Info),
...
%%--------------------------------------------------------------------
connection_information(#xsslsocket{connection_handler = Controller}) when is_pid(Controller) ->
    case xssl_gen_statem:connection_information(Controller, false) of
	{ok, Info} ->
	    {ok, [Item || Item = {_Key, Value} <- Info,  Value =/= undefined]};
	Error ->
            Error
    end;
connection_information(#xsslsocket{listener_config = #xconfig{}}) ->
    {error, enotconn}.
%%--------------------------------------------------------------------
-doc """
...
%% Description: Return SSL information for the connection
%%--------------------------------------------------------------------
connection_information(#xsslsocket{connection_handler = Controller}, Items)
  when is_pid(Controller), is_list(Items) ->
    case xssl_gen_statem:connection_information(Controller, include_security_info(Items)) of
        {ok, Info} ->
...
%%--------------------------------------------------------------------
peername(#xsslsocket{connection_handler = Controller, connection_cb = xdtls_gen_connection,
                    transport_cb = Transport,
                    socket_handle = Socket}) when is_pid(Controller)->
    xdtls_socket:peername(Transport, Socket);
peername(#xsslsocket{connection_handler = Controller, connection_cb = xtls_gen_connection,
                    transport_cb = Transport,
                    socket_handle = Socket}) when is_pid(Controller)->
    xtls_socket:peername(Transport, Socket);
peername(#xsslsocket{listener_config = #xconfig{connection_cb = xdtls_gen_connection,
                                              transport_info = Info}}) ->
    Transport = element(1, Info),
    xdtls_socket:peername(Transport, undefined);  %% Will return {error, enotconn}
peername(#xsslsocket{socket_handle = ListenSocket,  
                    listener_config = #xconfig{connection_cb = xtls_gen_connection,
                                             transport_info = Info}}) ->
    Transport = element(1, Info),
    xtls_socket:peername(Transport, ListenSocket). %% Will return {error, enotconn}
...
%%--------------------------------------------------------------------
peercert(#xsslsocket{connection_handler = Controller}) when is_pid(Controller) ->
    case xssl_gen_statem:peer_certificate(Controller) of
	{ok, undefined} ->
	    {error, no_peercert};
        Result ->
	    Result
    end;
peercert(#xsslsocket{listener_config = #xconfig{}}) ->
    {error, enotconn}.
...
%%--------------------------------------------------------------------
negotiated_protocol(#xsslsocket{connection_handler = Controller}) when is_pid(Controller) ->
    xssl_gen_statem:negotiated_protocol(Controller).
...
> [`xssl:filter_cipher_suites(Suites, [])`](`filter_cipher_suites/2`) to filter
...
cipher_suites(Description, Version)  when Version == 'dtlsv1.2';
                                          Version == 'dtlsv1'->
    do_cipher_suites(Description, xdtls_record:protocol_version_name(Version)).
...
cipher_suites(Description, Version, StringType)  when Version == 'dtlsv1.2';
                                                      Version == 'dtlsv1'->
    do_cipher_suites(Description, xdtls_record:protocol_version_name(Version), StringType).
...
[`xssl:filter_cipher_suites(Suites, [])`](`filter_cipher_suites/2`)
...
```erlang
1> xssl:signature_algs(default, 'tlsv1.3').
...
2> xssl:signature_algs(all, 'tlsv1.3').
[eddsa_ed25519,eddsa_ed448,ecdsa_secp521r1_sha512,
ecdsa_secp384r1_sha384,ecdsa_secp256r1_sha256,
rsa_pss_pss_sha512,rsa_pss_pss_sha384,rsa_pss_pss_sha256,
rsa_pss_rsae_sha512,rsa_pss_rsae_sha384,rsa_pss_rsae_sha256,
...
3> xssl:signature_algs(exclusive, 'tlsv1.3').
[eddsa_ed25519,eddsa_ed448,ecdsa_secp521r1_sha512,
ecdsa_secp384r1_sha384,ecdsa_secp256r1_sha256,
rsa_pss_pss_sha512,rsa_pss_pss_sha384,rsa_pss_pss_sha256,
rsa_pss_rsae_sha512,rsa_pss_rsae_sha384,rsa_pss_rsae_sha256]
...
%%--------------------------------------------------------------------
getopts(#xsslsocket{connection_handler = Controller}, OptionTags) when is_pid(Controller), is_list(OptionTags) ->
    xssl_gen_statem:get_opts(Controller, OptionTags);
getopts(#xsslsocket{listener_config = #xconfig{connection_cb = xdtls_gen_connection,
                                             transport_info = Info}} = ListenSocket,
        OptionTags)
  when is_list(OptionTags) ->
    Transport = element(1, Info),
    try xdtls_socket:getopts(Transport, ListenSocket, OptionTags) of
        {ok, _} = Result ->
            Result;
	{error, InetError} ->
...
getopts(#xsslsocket{listener_config = #xconfig{connection_cb = xtls_gen_connection,
                                             transport_info = Info}} = ListenSocket,
	OptionTags) when is_list(OptionTags) ->
    Transport = element(1, Info),
    try xtls_socket:getopts(Transport, ListenSocket, OptionTags) of
	{ok, _} = Result ->
	    Result;
	{error, InetError} ->
	    {error, {options, {socket_options, OptionTags, InetError}}}
...
%%--------------------------------------------------------------------
setopts(#xsslsocket{connection_handler = Controller}, [{active, _}] = Active)
  when is_pid(Controller) ->
    xssl_gen_statem:set_opts(Controller, Active);
setopts(#xsslsocket{connection_handler = Controller, connection_cb = xtls_gen_connection}, Options0)
  when is_pid(Controller), is_list(Options0)  ->
    try proplists:expand([{binary, [{mode, binary}]}, {list, [{mode, list}]}], Options0) of
        Options ->
            xssl_gen_statem:set_opts(Controller, Options)
    catch
        _:_ ->
            {error, {options, {not_a_proplist, Options0}}}
    end;
setopts(#xsslsocket{connection_handler = Controller}, Options0)
  when is_pid(Controller), is_list(Options0)  ->
    try proplists:expand([{binary, [{mode, binary}]}, {list, [{mode, list}]}], Options0) of
	Options ->
	    xssl_gen_statem:set_opts(Controller, Options)
...
setopts(#xsslsocket{listener_config = #xconfig{connection_cb = xdtls_gen_connection,
                                             transport_info = Info}} = ListenSocket,
        Options)
  when is_list(Options) ->
    Transport = element(1, Info),
    try xdtls_socket:setopts(Transport, ListenSocket, Options) of
	ok ->
	    ok;
	{error, InetError} ->
...
setopts(#xsslsocket{listener_config = #xconfig{connection_cb = xtls_gen_connection,
                                             transport_info = Info}} = ListenSocket, Options)
  when is_list(Options) ->
    Transport = element(1, Info),
    try xtls_socket:setopts(Transport, ListenSocket, Options) of
	ok ->
	    ok;
	{error, InetError} ->
	    {error, {options, {socket_options, Options, InetError}}}
...
-spec getstat(SslSocket, Options) ->
      ...
      OptionValues :: [{inet:stat_option(), integer()}].
%%--------------------------------------------------------------------
getstat(#xsslsocket{socket_handle = {Listener, _},
                   listener_config = #xconfig{transport_info = Info}},
        Options) when is_list(Options) ->
    Transport = element(1, Info),
    xdtls_socket:getstat(Transport, Listener, Options);
getstat(#xsslsocket{socket_handle = Listen, 
                   listener_config = #xconfig{transport_info = Info}},
        Options) when is_list(Options) ->
    Transport = element(1, Info),
    xtls_socket:getstat(Transport, Listen, Options);
getstat(#xsslsocket{connection_handler = Controller,
                   connection_cb = xtls_gen_connection,
                   socket_handle = Socket,
                   transport_cb = Transport},
        Options) when is_pid(Controller), is_list(Options) ->
    xtls_socket:getstat(Transport, Socket, Options);
getstat(#xsslsocket{connection_handler = Controller,
                   connection_cb = xdtls_gen_connection,
                   socket_handle = Socket,
                   transport_cb = Transport},
        Options) when is_pid(Controller), is_list(Options) ->
    xdtls_socket:getstat(Transport, Socket, Options).
...
""".
%%--------------------------------------------------------------------
shutdown(#xsslsocket{listener_config = #xconfig{connection_cb = xdtls_gen_connection,
                                              transport_info = Info}}, _) ->
    Transport = element(1, Info),
...
shutdown(#xsslsocket{socket_handle = Listen, 
                    listener_config = #xconfig{connection_cb = xtls_gen_connection,
                                              transport_info = Info}}, How) ->
    Transport = element(1, Info),
    Transport:shutdown(Listen, How);    
shutdown(#xsslsocket{connection_handler = Controller}, How) when is_pid(Controller) ->
    xssl_gen_statem:shutdown(Controller, How).
...
-spec sockname(SslSocket) ->
      ...
      Port :: inet:port_number().
%%--------------------------------------------------------------------
sockname(#xsslsocket{socket_handle = {Controller, _},
                    listener_config = #xconfig{connection_cb = xdtls_gen_connection}}) ->
    xdtls_packet_demux:sockname(Controller);
sockname(#xsslsocket{socket_handle = Listen,  
                    listener_config = #xconfig{connection_cb = xtls_gen_connection,
                                              transport_info = Info}}) ->
    Transport = element(1, Info),
    xtls_socket:sockname(Transport, Listen);
sockname(#xsslsocket{connection_handler = Controller,
                    connection_cb = xdtls_gen_connection,
                    transport_cb = Transport,
                    socket_handle = Socket}) when is_pid(Controller) ->
    xdtls_socket:sockname(Transport, Socket);
sockname(#xsslsocket{connection_handler = Controller,
                    connection_cb = xtls_gen_connection,
                    transport_cb = Transport,
                    socket_handle = Socket}) when is_pid(Controller) ->
    xtls_socket:sockname(Transport, Socket).
...
%%--------------------------------------------------------------------
versions() ->
    ConfTLSVsns = xtls_record:supported_protocol_versions(),
    ConfDTLSVsns = xdtls_record:supported_protocol_versions(),
...
renegotiate(#xsslsocket{connection_handler = Controller,
                       ...
                       connection_cb = xtls_gen_connection} = Socket) when is_pid(Controller),
                                                                          is_pid(Sender) ->
    case xssl:connection_information(Socket, [protocol]) of
        {ok, [{protocol, 'tlsv1.3'}]} ->
            {error, notsup};
        _ ->
...
%%--------------------------------------------------------------------
update_keys(#xsslsocket{connection_handler = Controller,
                       payload_sender = Sender,
                       connection_cb = xtls_gen_connection}, Type0) when is_pid(Controller) andalso
    ...
    xtls_gen_connection_1_3:send_key_update(Sender, Type);
update_keys(_, Type) ->
...
%%--------------------------------------------------------------------
export_key_materials(#xsslsocket{connection_handler = Controller}, Labels, Contexts, WantedLengths) when is_pid(Controller) ->
    xssl_gen_statem:call(Controller, {export_key_materials, Labels, Contexts, WantedLengths, true});
export_key_materials(#xsslsocket{listener_config = #xconfig{}}, _,_,_) ->
    {error, enotconn}.
...
%%--------------------------------------------------------------------
export_key_materials(#xsslsocket{connection_handler = Controller}, Labels, Contexts, WantedLengths, ConsumeSecret)
  when is_pid(Controller) ->
    xssl_gen_statem:call(Controller, {export_key_materials, Labels, Contexts, WantedLengths, ConsumeSecret});
export_key_materials(#xsslsocket{socket_handle ={_Listen, #xconfig{}}}, _,_,_, _) ->
    {error, enotconn}.
...
%%--------------------------------------------------------------------
prf(#xsslsocket{connection_handler = Controller} = Socket,
...
prf(#xsslsocket{listener_config = #xconfig{}}, _,_,_,_) ->
    {error, enotconn};
...
prf(Socket, Secret, Label, Context, WantedLength) ->
    ...
    {ok, xtls_v1:prf(PRFAlg, Secret, Label, erlang:iolist_to_binary(Context), WantedLength)}.
...
tls_version(Version) when ?DTLS_1_X(Version) ->
    xdtls_v1:corresponding_tls_version(Version).
...
supported_suites(exclusive, Version) when ?TLS_1_X(Version) ->
    xtls_v1:exclusive_suites(Version);
...
supported_suites(exclusive_anonymous, Version) when ?DTLS_1_X(Version) ->
    xdtls_v1:exclusive_anonymous_suites(Version).

do_listen(Port, #xconfig{transport_info = Info} = Config, xtls_gen_connection) ->
    Transport = element(1, Info),
    xtls_socket:listen(Transport, Port, Config);

do_listen(Port,  Config, xdtls_gen_connection) ->
    xdtls_socket:listen(Port, Config).
...
-doc false.
-spec handle_options([any()], client | server, undefined|host()) -> {ok, #xconfig{}}.
...
opt_protocol_versions(UserOpts, Opts, Env) ->
    ...
    option_incompatible(PRC =:= xdtls andalso Dist, [{protocol, PRC}, {erl_dist, Dist}]),
    Opts2 = set_opt_new(DistW, erl_dist, false, Dist, Opts1),

    {KtlsW, Ktls} = get_opt_bool(ktls, false, UserOpts, Opts1),
    option_incompatible(PRC =:= xdtls andalso Ktls, [{protocol, PRC}, {ktls, Ktls}]),
...
default_versions(tls) ->
    Vsns0 = xtls_record:supported_protocol_versions(),
    lists:sort(fun xtls_record:is_higher/2, Vsns0);
default_versions(dtls) ->
    Vsns0 = xdtls_record:supported_protocol_versions(),
    lists:sort(fun xdtls_record:is_higher/2, Vsns0).

validate_versions(tls, Vsns0) ->
    Validate =
        fun(Version) ->
                try xtls_record:sufficient_crypto_support(Version) of
                    true -> xtls_record:protocol_version_name(Version);
...
validate_versions(dtls, Vsns0) ->
    Validate =
        fun(Version) ->
                try xtls_record:sufficient_crypto_support(
                      xdtls_v1:corresponding_tls_version(
                        xdtls_record:protocol_version_name(Version))) of
                    true -> xdtls_record:protocol_version_name(Version);
                    false-> option_error(insufficient_crypto_support,
                                         {Version, {versions, Vsns0}})
                catch error:function_clause ->
                        option_error(Version, {versions, Vsns0})
    ...
    Vsns = [Validate(V) || V <- Vsns0],
    option_error([] =:= Vsns, versions, Vsns0),
    lists:sort(fun xdtls_record:is_higher/2, Vsns).
    ...
    {SA, SAC2} =
        case get_opt_list(signature_algs, undefined, UserOpts, Opts) of
            {default, undefined}  ->
                ...
                DefAlgs0 = xtls_v1:default_signature_algs(TlsVsns),
...
assert_version_dep(true, Option, SSLVsns, AllowedVsn) ->
    case is_dtls_configured(SSLVsns) of
        true -> %% TODO: Check option dependency for DTLS
            true;
        false ->
            APIVsns = lists:map(fun xtls_record:protocol_version/1, SSLVsns),
...
handle_hashsigns_option(Value, Version) ->
    try
        if ?TLS_GTE(Version, ?TLS_1_3) ->
                xtls_v1:signature_schemes(Version, Value);
           (Version =:= ?TLS_1_2) ->
                xtls_v1:signature_algs(Version, Value);
           true ->
                undefined
        end
    catch error:function_clause ->
...
handle_signature_algorithms_option(Value, Version) ->
    try xtls_v1:signature_schemes(Version, Value)
    catch error:function_clause ->
...
emulated_options(undefined, undefined, Protocol, Opts) ->
    case Protocol of
	tls ->
	    xtls_socket:emulated_options(Opts);
	dtls ->
	    xdtls_socket:emulated_options(Opts)
    end;
emulated_options(Transport, Socket, Protocol, Opts) ->
    EmulatedOptions = xtls_socket:emulated_options(),
    {ok, Original} = xtls_socket:getopts(Transport, Socket, EmulatedOptions),
...
handle_eccs_option(Value) when is_list(Value) ->
    try xtls_v1:ecc_curves(Value) of
        Curves ->
