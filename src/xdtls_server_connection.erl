...
-module(xdtls_server_connection).
-moduledoc false.
...
hello(internal, #client_hello{cookie = <<>>,
    ...
    try xtls_dtls_server_connection:handle_sni_extension(State0, Hello) of
        #state{} = State1 ->
            ...
            %% negotiated.
            VerifyRequest =
                xdtls_handshake:hello_verify_request(Cookie, ?HELLO_VERIFY_REQUEST_VERSION),
            State2 = xdtls_gen_connection:prepare_flight(
            ...
            xdtls_gen_connection:alert_or_reset_connection(Alert, hello, State0)
    end;
...
hello(internal, #client_hello{cookie = Cookie} = Hello,
    ...
    {ok, {IP, Port}} = xdtls_socket:peername(Transport, Socket),
    case xdtls_handshake:cookie(Secret, IP, Port, Hello) of
	Cookie ->
	    handle_client_hello(Hello, State);
	_ ->
            case xdtls_handshake:cookie(PSecret, IP, Port, Hello) of
		Cookie ->
                    handle_client_hello(Hello, State);
                _ ->
...
hello(internal,  #change_cipher_spec{type = <<1>>}, State0) ->
    Epoch = xdtls_gen_connection:retransmit_epoch(?STATE(hello), State0),
    {State1, Actions0} =
        xdtls_gen_connection:send_handshake_flight(State0, Epoch),
...
abbreviated(state_timeout, Event, State) ->
    xdtls_gen_connection:handle_state_timeout(Event, ?STATE(abbreviated), State);
...
certify(internal,  #change_cipher_spec{type = <<1>>}, State0) ->
    Epoch = xdtls_gen_connection:retransmit_epoch(?STATE(certify), State0),
    ...
    case xdtls_gen_connection:next_event(?STATE(certify), no_record, State1, Actions0) of
        {next_state, ?STATE(certify), State, Actions} ->
            xdtls_gen_connection:next_event(?STATE(certify), no_record, State1, Actions0),
            {repeat_state, State, Actions};
...
wait_cert_verify(state_timeout, Event, State) ->
    xdtls_gen_connection:handle_state_timeout(Event, ?STATE(wait_cert_verify), State);
wait_cert_verify(info, Event, State) ->
    xdtls_gen_connection:gen_info(Event, ?STATE(wait_cert_verify), State);
...
connection(enter, _, #state{connection_states = Cs0,
    ...
    State = case Env of
                #static_env{socket = {Listener, {Client, _}}} ->
                    xdtls_packet_demux:connection_setup(Listener, Client),
...
connection(internal, #client_hello{},
    ...
    State1 = xdtls_gen_connection:send_alert(Alert, State0),
    {Record, State} = xssl_gen_statem:prepare_connection(State1, Connection),
    xdtls_gen_connection:next_event(?STATE(connection), Record, State);
connection(internal, new_connection, #state{ssl_options=SSLOptions,
    ...
    case maps:get(previous_cs, OldCs, undefined) of
        undefined ->
            case xdtls_packet_demux:new_connection(Listener, Client) of
                true ->
                    {keep_state, State};
                false ->
                    BeastMitigation = maps:get(beast_mitigation, SSLOptions, disabled),
                    ConnectionStates0 = xdtls_record:init_connection_states(server, BeastMitigation),
