...
encode_data(Data, Version, ConnectionStates) ->
	...
	N when N > MaxLength ->
            Frags = xtls_record:split_iovec(erlang:iolist_to_iovec(Data), MaxLength),
...
supported_protocol_versions([]) ->
    ...
    application:set_env(ssl, xdtls_protocol_version, Vsns),
    Vsns;
...
encode_plain_text(Type, Version, Fragment, #{epoch := Epoch,
    ...
    TLSVersion = xdtls_v1:corresponding_tls_version(Version),
    {CipherFrag, CipherS1} = xssl_cipher:cipher(BulkCipherAlgo, CipherS0, MAC, Fragment, TLSVersion),
...
mac_hash(Version, MacAlg, MacSecret, Epoch, SeqNo, Type, Length, Fragment) ->
    ...
    Value = [<<?UINT16(Epoch), ?UINT48(SeqNo), ?BYTE(Type),
     ...
     Fragment],
    xdtls_v1:hmac_hash(MacAlg, MacSecret, Value).
