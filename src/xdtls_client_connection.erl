...
-module(xdtls_client_connection).
-moduledoc false.
...
-include("xdtls_connection.hrl").
-include("xdtls_handshake.hrl").
...
init([Role, Tab, Host, Port, Socket, Options,  User, CbInfo]) ->
    ...
    State0 = xdtls_gen_connection:initial_state(Role, Tab, Host, Port, Socket,
                                               Options, User, CbInfo),
...
initial_hello({call, From}, {start, Timeout},
    ...
    HelloVersion = xdtls_record:hello_version(Version, Versions),
    State1 = xdtls_gen_connection:prepare_flight(
               State0#state{connection_env = CEnv#connection_env{negotiated_version = Version},
                            connection_states = ConnectionStates1}),
    {State2, Actions} =
        xdtls_gen_connection:send_handshake(Hello,
    ...
    xdtls_gen_connection:next_event(hello, no_record, State,
...
initial_hello({call, From}, {start, {Opts, EmOpts}, Timeout},
    ...
    try
        SslOpts = xssl:update_options(Opts, Role, OrigSSLOptions),
...
initial_hello(info, Event, State) ->
    xdtls_gen_connection:gen_info(Event, ?STATE(initial_hello), State);
initial_hello(Type, Event, State) ->
    xssl_gen_statem:initial_hello(Type, Event, State).
...
hello(internal, #hello_verify_request{cookie = Cookie},
    ...
    OcspNonce = xtls_handshake:ocsp_nonce(SslOpts),
    Hello = xdtls_handshake:client_hello(Host, Port, Cookie, ConnectionStates0,
    ...
    xdtls_gen_connection:next_event(?STATE(hello), no_record, State, Actions);
    ...
    try
        ...
        xtls_dtls_client_connection:handle_session(
          ...
          State#state{handshake_env =
            ...
            xssl_gen_statem:handle_own_alert(Alert, hello, State)
    end;
...
hello(internal,  #change_cipher_spec{type = <<1>>}, State0) ->
    Epoch = xdtls_gen_connection:retransmit_epoch(hello, State0),
    {State1, Actions0} = xdtls_gen_connection:send_handshake_flight(State0, Epoch),
    %% This will reset the retransmission timer by repeating the enter state event
    case xdtls_gen_connection:next_event(?STATE(hello), no_record, State1, Actions0) of
...
hello(state_timeout, Event, State) ->
    xdtls_gen_connection:handle_state_timeout(Event, ?STATE(hello), State);
hello(info, Event, State) ->
    xdtls_gen_connection:gen_info(Event, ?STATE(hello), State);
...
user_hello(info, Event, State) ->
    xdtls_gen_connection:gen_info(Event, ?STATE(user_hello), State);
...
abbreviated(state_timeout, Event, State) ->
    xdtls_gen_connection:handle_state_timeout(Event, abbreviated, State);
abbreviated(info, Event, State) ->
    xdtls_gen_connection:gen_info(Event, ?STATE(abbreviated), State);
...
wait_stapling(state_timeout, Event, State) ->
    xdtls_gen_connection:handle_state_timeout(Event, ?STATE(wait_stapling), State);
wait_stapling(info, Event, State) ->
    xdtls_gen_connection:gen_info(Event, ?STATE(wait_stapling), State);
...
certify(internal,  #change_cipher_spec{type = <<1>>}, State0) ->
    Epoch = xdtls_gen_connection:retransmit_epoch(certify, State0),
    {State1, Actions0} = xdtls_gen_connection:send_handshake_flight(State0, Epoch),
    %% This will reset the retransmission timer by repeating the enter state event
    case xdtls_gen_connection:next_event(?STATE(certify), no_record, State1, Actions0) of
...
certify(state_timeout, Event, State) ->
    xdtls_gen_connection:handle_state_timeout(Event, ?STATE(certify), State);
certify(info, Event, State) ->
    xdtls_gen_connection:gen_info(Event, ?STATE(certify), State);
...
cipher(info, Event, State) ->
    xdtls_gen_connection:gen_info(Event, ?STATE(cipher), State);
cipher(state_timeout, Event, State) ->
    xdtls_gen_connection:handle_state_timeout(Event, ?STATE(cipher), State);
...
connection(internal, #hello_request{},
    ...
    State = State2#state{protocol_specific =
                             PS#{flight_state => xdtls_gen_connection:initial_flight_state(DataTag)},
                         session = Session},
    xdtls_gen_connection:next_event(hello, no_record, State, Actions);
connection(internal, #hello_request{},
	   #state{static_env = #static_env{host = Host,
                                           port = Port},
    ...
    #{current_cookie_secret := Cookie} = PS,
    Hello = xdtls_handshake:client_hello(Host, Port, Cookie, ConnectionStates,
					SslOpts,<<>>, Renegotiation, maps:get(ocsp_nonce, StaplingState, undefined)),
    {State, Actions} = xdtls_gen_connection:send_handshake(Hello, State0),
    xdtls_gen_connection:next_event(hello, no_record, State, Actions);
connection(internal, renegotiate, State0) ->
    State = #state{handshake_env = HsEnv} = xdtls_gen_connection:reinit_handshake_data(State0),
...
connection({call, From}, {application_data, Data}, State) ->
    try
        xdtls_gen_connection:send_application_data(Data, From, ?STATE(connection), State)
    catch throw:Error ->
            xssl_gen_statem:hibernate_after(?STATE(connection), State, [{reply, From, Error}])
    end;
connection({call, From}, {downgrade, Pid},
    ...
    xdtls_socket:setopts(Transport, Socket, [{active, false}, {packet, 0}, {mode, binary}]),
...
connection(info, Event, State) ->
    xdtls_gen_connection:gen_info(Event, ?STATE(connection), State);
...
downgrade(info, Event, State) ->
    xdtls_gen_connection:gen_info(Event, ?STATE(downgrade), State);
