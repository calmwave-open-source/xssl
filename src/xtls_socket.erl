...
listen(Transport, Port, #xconfig{transport_info = {Transport, _, _, _, _}, 
				inet_user = Options, 
	...
	{ok, ListenSocket} ->
	    {ok, Tracker} = inherit_tracker(ListenSocket, EmOpts, SslOpts),
            ...
            Socket = #xsslsocket{socket_handle = ListenSocket,
                                listener_config = Config#xconfig{trackers = Trackers}},
...
accept(ListenSocket, #xconfig{transport_info = {Transport,_,_,_,_} = CbInfo,
			     ssl = SslOpts,
...
upgrade(Socket, #xconfig{transport_info = {Transport,_,_,_,_}= CbInfo,
			ssl = SslOptions,
			emulated = EmOpts, connection_cb = ConnectionCb}, Timeout) ->
    ok = setopts(Transport, Socket, xtls_socket:internal_inet_values()),
    case peername(Transport, Socket) of
	{ok, {Address, Port}} ->
	    xssl_gen_statem:connect(ConnectionCb, Address, Port, Socket,
				   {SslOptions, 
...
connect(Address, Port,
	#xconfig{transport_info = CbInfo, inet_user = UserOpts, ssl = SslOpts,
		emulated = EmOpts, inet_ssl = SocketOpts, connection_cb = ConnetionCb},
	Timeout) ->
    {Transport, _, _, _, _} = CbInfo,
    try Transport:connect(Address, Port,  SocketOpts, Timeout) of
	{ok, Socket} ->
	    xssl_gen_statem:connect(ConnetionCb, Address, Port, Socket,
				   {SslOpts, 
				    emulated_socket_options(EmOpts, #xsocket_options{}), undefined},
				   self(), CbInfo, Timeout);
	{error, Reason} ->
...
setopts(gen_tcp, Socket = #xsslsocket{socket_handle = ListenSocket, 
                                     listener_config = #xconfig{trackers = Trackers}}, Options) ->
    Tracker = proplists:get_value(option_tracker, Trackers),
...
setopts(_, Socket = #xsslsocket{socket_handle = ListenSocket, 
...
check_active_n(EmulatedOpts, Socket = #xsslsocket{listener_config = #xconfig{trackers = Trackers}}) ->
    Tracker = proplists:get_value(option_tracker, Trackers),
...
getopts(gen_tcp,  #xsslsocket{socket_handle = ListenSocket, 
                             listener_config = #xconfig{trackers = Trackers}}, Options) ->
    Tracker = proplists:get_value(option_tracker, Trackers),
...
getopts(Transport,  #xsslsocket{socket_handle = ListenSocket, 
                               listener_config = #xconfig{trackers = Trackers}}, Options) ->
    Tracker = proplists:get_value(option_tracker, Trackers),
    {SockOptNames, EmulatedOptNames} = split_options(Options),
    EmulatedOpts = get_emulated_opts(Tracker, EmulatedOptNames),
...
session_tickets_tracker(ListenSocket, Lifetime, TicketStoreSize, MaxEarlyDataSize,
    ...
    SupName = xtls_server_session_ticket_sup:sup_name(dist),
    Children = supervisor:count_children(SupName),
    Workers = proplists:get_value(workers, Children),
    case Workers of
        0 ->
            xtls_server_session_ticket_sup:start_child([ListenSocket, Mode, Lifetime,
...
%%--------------------------------------------------------------------
-spec handle_info(term(), #state{}) ->  {stop, xssl:reason(), #state{}}.
%% Possible return values not used now.
%%			              {noreply, #state{}}.
%%				      |{noreply, #state{}, timeout()} |
%%				     
...
%%--------------------------------------------------------------------
-spec terminate(xssl:reason(), #state{}) -> ok.
%%		       
...
start_tls_server_connection(SslOpts, Port, Socket, EmOpts, Trackers, CbInfo) ->
    try
        {ok, DynSup} = xtls_connection_sup:start_child([]),
        SenderOpts = maps:get(sender_spawn_opts, SslOpts, []),
        {ok, Sender} = xtls_dyn_connection_sup:start_child(DynSup, sender, [[{spawn_opt, SenderOpts}]]),
        ConnArgs = [server, Sender, "localhost", Port, Socket,
                    {SslOpts, emulated_socket_options(EmOpts, #xsocket_options{}), Trackers}, self(), CbInfo],
        {ok, Pid} = xtls_dyn_connection_sup:start_child(DynSup, receiver, ConnArgs),
        receive {Pid, user_socket, UserSocket} ->
                xssl_gen_statem:socket_control(UserSocket)
        end
    catch
