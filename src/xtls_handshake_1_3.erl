...
maybe_add_cookie_extension(#state{connection_states = ConnectionStates,
    ...
    Cookie = xssl_cipher:encrypt_data(<<"cookie">>, MessageHash, Shard, IV),
...
validate_finished(#state{connection_states = ConnectionStates,
    ...
    ControlData = xtls_v1:finished_verify_data(FinishedKey, HKDFAlgo, Messages),
...
encode_early_data(Cipher,
    ...
    BulkCipherAlgo = xssl_cipher:bulk_cipher_algorithm(Cipher),
    ...
    {BinEarlyData, ConnectionStates} = xtls_record:encode_data([EarlyData], Version, ConnectionStates1),
...
decode_handshake(?CERTIFICATE_VERIFY, <<?UINT16(EncAlgo), ?UINT16(Size), Signature:Size/binary>>) ->
    Algorithm = xssl_cipher:signature_scheme(EncAlgo),
    #certificate_verify_1_3{
       algorithm = Algorithm,
       signature = Signature};
...
encode_algorithm(Algo) ->
    Scheme = xssl_cipher:signature_scheme(Algo),
    <<?UINT16(Scheme)>>.
...
message_hash(ClientHello1, HKDFAlgo) ->
    [?MESSAGE_HASH,
     0,0,xssl_cipher:hash_size(HKDFAlgo),
...
calculate_handshake_secrets(PublicKey, PrivateKey, SelectedGroup, PSK,
    ...
    HandshakeSecret = xtls_v1:key_schedule(handshake_secret, HKDFAlgo, IKM, EarlySecret),

    %% Calculate [sender]_handshake_traffic_secret
    {Messages, _} =  HHistory,
    ClientHSTrafficSecret =
        xtls_v1:client_handshake_traffic_secret(HKDFAlgo, HandshakeSecret, lists:reverse(Messages)),
    ServerHSTrafficSecret =
        xtls_v1:server_handshake_traffic_secret(HKDFAlgo, HandshakeSecret, lists:reverse(Messages)),

    %% Calculate traffic keys
    KeyLength = xtls_v1:key_length(CipherSuite),
    {ReadKey, ReadIV} = xtls_v1:calculate_traffic_keys(HKDFAlgo, KeyLength, ClientHSTrafficSecret),
    {WriteKey, WriteIV} = xtls_v1:calculate_traffic_keys(HKDFAlgo, KeyLength, ServerHSTrafficSecret),

    %% Calculate Finished Keys
    ReadFinishedKey = xtls_v1:finished_key(ClientHSTrafficSecret, HKDFAlgo),
    WriteFinishedKey = xtls_v1:finished_key(ServerHSTrafficSecret, HKDFAlgo),
...
calculate_client_early_traffic_secret(
  ...
  #state{connection_states = ConnectionStates,
    ...
    ClientEarlyTrafficSecret =
        xtls_v1:client_early_traffic_secret(HKDFAlgo, EarlySecret, ClientHello),
    %% Calculate traffic key
    KeyLength = xssl_cipher:key_material(Cipher),
    {Key, IV} =
        xtls_v1:calculate_traffic_keys(HKDFAlgo, KeyLength, ClientEarlyTrafficSecret),
...
%%     binary:copy(<<0>>, xssl_cipher:hash_size(HKDFAlgo));
...
get_pre_shared_key(_, _, HKDFAlgo, undefined) ->
    {ok, binary:copy(<<0>>, xssl_cipher:hash_size(HKDFAlgo))};
...
get_pre_shared_key(auto = SessionTickets, UseTicket, HKDFAlgo, ServerPSK) ->
    ...
    case choose_psk(TicketData, ServerPSK) of
        undefined -> %% full handshake, default PSK
            xtls_client_ticket_store:unlock_tickets(self(), UseTicket),
            {ok, binary:copy(<<0>>, xssl_cipher:hash_size(HKDFAlgo))};
        illegal_parameter ->
            xtls_client_ticket_store:unlock_tickets(self(), UseTicket),
            {error, ?ALERT_REC(?FATAL, ?ILLEGAL_PARAMETER)};
        {Key, PSK, _, _, _} ->
            xtls_client_ticket_store:remove_tickets([Key]),  %% Remove single-use ticket
            xtls_client_ticket_store:unlock_tickets(self(), UseTicket -- [Key]),
            {ok, PSK}
    end.
%%
%% Early Data
...
calculate_traffic_secrets(#state{
    ...
    MasterSecret =
        xtls_v1:key_schedule(master_secret, HKDFAlgo, HandshakeSecret),
    ...
    ClientAppTrafficSecret0 =
        xtls_v1:client_application_traffic_secret_0(HKDFAlgo, MasterSecret, lists:reverse(Messages)),
    ServerAppTrafficSecret0 =
        xtls_v1:server_application_traffic_secret_0(HKDFAlgo, MasterSecret, lists:reverse(Messages)),

    %% Calculate traffic keys
    KeyLength = xtls_v1:key_length(CipherSuite),
    {ReadKey, ReadIV} = xtls_v1:calculate_traffic_keys(HKDFAlgo, KeyLength, ClientAppTrafficSecret0),
    {WriteKey, WriteIV} = xtls_v1:calculate_traffic_keys(HKDFAlgo, KeyLength, ServerAppTrafficSecret0),
...
maybe_calculate_resumption_master_secret(#state{
  ...
  when SessionTickets =/= disabled  ->
    ...
    RMS = xtls_v1:resumption_master_secret(HKDFAlgo, MasterSecret, lists:reverse(Messages0)),
...
calculate_exporter_master_secret(#state{
    ...
    Messages = get_handshake_context(Role, HHistory),
    xtls_v1:exporter_master_secret(HKDFAlgo, MasterSecret, lists:reverse(Messages)).
...
update_start_state(#state{connection_states = ConnectionStates0,
    ...
    SecParamsR1 = xssl_cipher:security_parameters_1_3(SecParamsR0, Cipher),
    SecParamsW1 = xssl_cipher:security_parameters_1_3(SecParamsW0, Cipher),
...
select_sign_algo(PublicKeyAlgo, RSAKeySize, [CertSignAlg|CertSignAlgs], OwnSignAlgs, Curve) ->
    {_, S, _} = xssl_cipher:scheme_to_components(CertSignAlg),
...
validate_key_compatibility(PublicKeyAlgo, RSAKeySize, [CertSignAlg|CertSignAlgs], OwnSignAlgs, Curve)
  when PublicKeyAlgo =:= rsa orelse
    ...
    {Hash, Sign, _} = xssl_cipher:scheme_to_components(CertSignAlg),
...
validate_key_compatibility(PublicKeyAlgo, RSAKeySize, [CertSignAlg|CertSignAlgs], OwnSignAlgs, Curve)
  when PublicKeyAlgo =:= ecdsa ->
    {_ , Sign, PeerCurve} = xssl_cipher:scheme_to_components(CertSignAlg),
...
is_rsa_key_compatible(KeySize, Hash) ->
    HashSize = xssl_cipher:hash_size(Hash),
...
do_check_cert_sign_algo(SignAlgo, SignHash, [Scheme|T]) ->
    ...
    {Hash, Sign, _Curve} = xssl_cipher:scheme_to_components(Scheme),
...
maybe_add_binders(Hello0, TicketData, ?TLS_1_3=Version) ->
    HelloBin0 = xtls_handshake:encode_handshake(Hello0, Version),
...
create_binders(_, [], Acc) ->
    lists:reverse(Acc);
create_binders(Context, [#xticket_data{
                            psk = PSK,
...
truncate_client_hello(HelloBin0) ->
    ...
    CH0 = #client_hello{
        ...
        xtls_handshake:decode_handshake(?TLS_1_3, Type, Body),
...
calculate_finished_key(PSK, HKDFAlgo) ->
    EarlySecret = xtls_v1:key_schedule(early_secret, HKDFAlgo , {psk, PSK}),
    PRK = xtls_v1:resumption_binder_key(HKDFAlgo, EarlySecret),
    xtls_v1:finished_key(PRK, HKDFAlgo).


calculate_binder(BinderKey, HKDF, Truncated) ->
    xtls_v1:finished_verify_data(BinderKey, HKDF, [Truncated]).
...
get_ticket_data(Pid, auto, UseTicket) ->
    xtls_client_ticket_store:get_tickets(Pid, UseTicket).
