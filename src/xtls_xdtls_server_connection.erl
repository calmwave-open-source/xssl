...
initial_hello({call, _From} = Type, {new_user, _} = Msg, State) ->
    xssl_gen_statem:initial_hello(Type, Msg, State);
...
wait_cert_verify({call, From}, Msg, State) ->
    xssl_gen_statem:handle_call(Msg, From, ?STATE(wait_cert_verify), State);
...
%%====================================================================
%% Help functions for xtls|dtls_server_connection.erl
%%====================================================================
...
handle_sni_extension(#state{static_env =
    ...
    case xssl_gen_statem:handle_sni_extension(PossibleSNI, State0) of
        {ok, State} ->
            State;
        {error, #alert{}=Alert} ->
            throw(Alert)
