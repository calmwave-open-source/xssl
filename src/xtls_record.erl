...
encode_handshake(Frag, ?TLS_1_3, ConnectionStates) ->
    xtls_record_1_3:encode_handshake(Frag, ConnectionStates);
...
encode_alert_record(Alert, ?TLS_1_3, ConnectionStates) ->
    xtls_record_1_3:encode_alert_record(Alert, ConnectionStates);
...
encode_data(Data, ?TLS_1_3, ConnectionStates) ->
    xtls_record_1_3:encode_data(Data, ConnectionStates);
...
decode_cipher_text(?TLS_1_3, CipherTextRecord, ConnectionStates, _) ->
    xtls_record_1_3:decode_cipher_text(CipherTextRecord, ConnectionStates);
        ...
        Length =< Max ->
            if
                Length =< Size0 ->
                    ...
                    xssl_logger:debug(get(log_level), inbound, 'record', Record),
                    case Downgrade of
                        {_Pid, _From} ->
