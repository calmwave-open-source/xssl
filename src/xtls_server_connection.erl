...
init([Role, Sender, Tab, Host, Port, Socket, Options,  User, CbInfo]) ->
    ...
    try
        State = xssl_gen_statem:init_ssl_config(State0#state.ssl_options, Role, State0),
        ...
        case choose_tls_fsm(SslOpts, Hello) of
            xtls_1_3_fsm ->
                {next_state, start, State1,
                 [{change_callback_module, xtls_server_connection_1_3},
            ...
            xssl_gen_statem:handle_own_alert(Alert, ?STATE(hello), AlertState)
    end;
hello(info, Event, State) ->
    xtls_gen_connection:handle_info(Event, ?STATE(hello), State);
...
wait_cert_verify(info, Event, State) ->
    xtls_gen_connection:gen_info(Event, ?STATE(wait_cert_verify), State);
...
wait_cert_verify(Type, Event, State) ->
    xssl_gen_statem:handle_common_event(Type, Event, ?STATE(wait_cert_verify), State).
...
connection(internal, #client_hello{} = Hello,
    ...
    {ok, Write} = xtls_sender:renegotiate(Sender),
    xtls_gen_connection:next_event(hello, no_record,
...
connection(internal, #client_hello{},
    ...
    xtls_gen_connection:send_alert_in_connection(Alert, State0),
    State = xtls_gen_connection:reinit_handshake_data(State0),
...
connection({call, From}, renegotiate, #state{static_env = #static_env{socket = Socket,
    ...
    xtls_gen_connection:next_event(hello, no_record, State);
