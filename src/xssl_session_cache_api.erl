...
-type session_cache_key() :: {partial_key(), xssl:session_id()}.
...
-opaque partial_key()     :: {xssl:host(), inet:port_number()} | inet:port_number().
...
-callback select_session(CacheRef, Server) -> Sessions when
      CacheRef :: session_cache_ref(),
      Server ::{xssl:host(), inet:port_number()} | inet:port_number(),
      Sessions :: [session()].
